...
import { useThinkingLoop } from './useThinkingLoop'
...
function dots(container: HTMLElement): HTMLElement[] {
  return Array.from(container.querySelectorAll('.thinking-dot'))
}

describe('ThinkingMark', () => {
  it('renders all 14 dots at rest when not streaming', () => {
    const { container } = render(<ThinkingMark isStreaming={false} />)
    const rendered = dots(container)
    expect(rendered).toHaveLength(14)
    expect(rendered.every((dot) => !dot.classList.contains('animate-thinking-dot'))).toBe(true)
  })

  it('animates every dot once streaming starts', () => {
    const { container } = render(<ThinkingMark isStreaming={true} />)
    const rendered = dots(container)
    expect(rendered).toHaveLength(14)
    expect(rendered.every((dot) => dot.classList.contains('animate-thinking-dot'))).toBe(true)
  })
...
describe('useThinkingLoop', () => {
  it('activates all 14 dots on start and stops only the dot whose own boundary fires', () => {
    const { result, rerender } = renderHook(({ isStreaming }) => useThinkingLoop(isStreaming), {
  ...
  it('ignores an iteration that fires before any stop was requested', () => {
    const { result } = renderHook(({ isStreaming }) => useThinkingLoop(isStreaming), {
      initialProps: { isStreaming: true }
    })
    const [firstId] = [...result.current.activeIds]

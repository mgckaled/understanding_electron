...
```
   ...
   15 orçamento de contexto e modelo   plan/active/15-...               passos 0–5 feitos
...
| ~~Vigésimo canal em `shared/ipc.ts`~~ **disparado em ago/2026** — `ai:loaded` e `ai:unload` levaram a conta a exatamente 20. Fica **aberto e por decidir**: separar de fato, ou concluir que a régua era o proxy errado. O argumento contra separar agora é que os 20 canais somam pouco mais de 100 linhas de contrato e a skill `architecture` os cobre em três parágrafos; o argumento a favor é que o plano 16 traz anexo, que é o primeiro canal com payload binário. **Tarefa própria, nunca no meio de outra** | Skill própria para IPC, separada de `architecture` | [`08-automacao`](plan/implemented/08-automacao-e-registro.md) |

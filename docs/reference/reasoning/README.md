...
**Fora de escopo aqui, por desenho:** persistência e reenvio da `signature` do raciocínio (o texto já aparece; a assinatura, quando ausente, só gera um log informativo) é o [`21-D-B`](../../plan/active/21-D-B-signature-resend-e-orcamento.md), ainda não implementado — `exposesReasoning()` já inclui Gemini desde o 21-D-A (D21D.4, puxado da decisão original que adiava para o 21-D-B).

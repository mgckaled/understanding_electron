...
- **21-C — orçamento sob raciocínio, faixas fixas, e o martelo do Gemini.** Implementado em 03/09/2026, três planos (revisão do advisor separou por não compartilharem código). ⚠️ **Verificação ao vivo por sub-plano, não uniforme — corrigido no R-6 (o texto anterior aqui dizia "verificado ao vivo no mesmo dia" para os três, o que o diário de cada plano não sustenta):** [`21-C-A`](../../plan/active/21-C-A-orcamento-de-geracao.md) tem verificação ao vivo **registrada e confirmada** no próprio diário (os 6 pontos do roteiro, inclusive o ancoramento pós-fato — headroom de geração reservado, calibração por média móvel, e o complemento D21C.12/D21C.13: o medidor nunca exibia `promptTokens`/`evalTokens` reais, só a estimativa de caracteres). [`21-C-B`](../../plan/active/21-C-B-motivo-de-parada.md) (`context-exhausted`) teve uma **sondagem ao vivo da API** (`curl` direto contra o Ollama, confirmou `done_reason:'length'`) — que é verificação do protocolo, não da volta completa na UI; o diário do próprio plano registra "verificação ao vivo pendente — fica com o usuário". [`21-C-C`](../../plan/active/21-C-C-faixas-fixas-de-contexto.md) (faixas fixas + campo numérico, `exposesReasoning` desliga o switch do Gemini, causa do D21A.10 fechada — § acima) tem o mesmo diário: "verificação ao vivo pendente — fica com o usuário".

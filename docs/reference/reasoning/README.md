...
O que motiva o painel, sem mudar (achado técnico, não a numeração): `measureChatTiming` (`src/main/observatory/chatTiming.ts`) marca `t1` (tempo até primeiro token) no primeiro `onChunk`, e deriva `decodeMs`/tokens-por-segundo do `evalTokens` (`eval_count` do Ollama) no fim do stream — hoje isso só mede conteúdo.

- se o primeiro `onChunk` a disparar passar a ser um chunk de *raciocínio*, o "tempo até primeiro token" do O-7 passa a significar outra coisa, silenciosamente — nenhuma linha do O-7 muda, mas o número passa a mentir sobre o que mede;
- `eval_count` do Ollama quase certamente conta **todos** os tokens gerados, raciocínio incluso — a taxa de tokens/s do O-7 passaria a **misturar** decode de raciocínio com decode de resposta, indistinguíveis.
...
- **21-A — o dado atravessa, o nome se resolve, e a UI existe (sem ser bonita ainda).** O rename `Thinking*` → `Responding*` (seção acima) entra **aqui**, antes de qualquer código novo usar `reasoning`. Depois: `ChatFn.onThinking?` aditivo (os três adaptadores e o `ai:propose` continuam compilando sem tocar); `JobEvent` ganha a variante `{ type: 'reasoning'; text }` (linha única — `job:event` não passa por `IpcContract`/`argsSchema`); os três adaptadores (Ollama e GLM triviais, Gemini com o filtro do achado #1); `reasoningPartSchema` em `shared/ipc.ts` + o `case 'reasoning':` de `partForProvider` (decide o retorno, recomendação `''`); hook novo no renderer (`useJobReasoning`, irmão de `useJobChunks.ts`); **o toggle não nasce — destrava-se**: a linha "Raciocínio visível" que já existe em `AttachButton.tsx:207-218` ganha `disabled={!hasCapability(model, 'thinking')}` e `onChange` real (seção acima), as outras duas linhas do mesmo array (`Busca web`, `Documentação (MCP)`) continuam como estão — arcos 22/23, fora de escopo aqui, e F2.6 já proíbe redesenhar a linha. Exibição em tela: texto corrido simples (sem colapsar, sem o polimento do 21-B) só para provar que o dado chega — local sugerido é a mesma banda que o `RespondingMark` isolou (seção acima), sem comprometer o desenho final. Default do toggle **desligado** (mesmo argumento medido do `CHAT_TIMEOUT_MS`: prefill mais lento nesta CPU) — o usuário escolhe ligar por conversa/turno, nunca o app decide por ele. Decidir explicitamente que `requestStructuredChat` (caminho do `ai:propose`) **fica de fora**; e o que fazer com o fallback de stream truncado do `ollamaChat` (hoje `return { content: assembled }`, sem contrapartida para raciocínio parcial).
- **21-B — o dado aparece bem.** Substitui a exibição mínima do 21-A pelo bloco recolhível de verdade (`Reasoning*`, no espaço que o layout isolado do `RespondingMark` abre), rótulo por provedor (achado #3), estado padrão colapsado/expandido durante o streaming (o padrão do mercado: aberto e rotulado "Pensando…"/"Raciocinando…" enquanto chega, recolhe para um resumo clicável ao terminar — pesquisar o comportamento exato na sessão que escrever o plano). Sem migração de banco a fazer (`parts` já é JSON) e sem UI a inventar do zero (o toggle já existia) — o que sobra aqui é desenho, então vale checar, ao escrever o plano, se ainda se justifica como arquivo próprio ou se cabe como continuação do 21-A na mesma sessão.
- **O-9 — Observatório**, condicional e fora da numeração do arco 21 (seção acima): só ganha arquivo se, ao chegar lá, o O-7 precisar mesmo separar as duas taxas de decode. Do contrário, vira parágrafo de decisão documentada dentro do 21-B, não um painel novo.
...
## O que ainda não foi verificado (fica para a sessão que escrever 21-A)
...
- Ao vivo: se `gemini-3.1-flash-lite`/`gemini-3.7-flash` (os dois modelos do catálogo) de fato devolvem `part.thought` em streaming nesta conta — o achado #2 é risco documentado, não medição própria.

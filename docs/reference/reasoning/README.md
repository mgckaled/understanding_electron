...
⚠️ **O mecanismo inteiro segue não verificado ao vivo, e por um motivo estrutural, não um bug:** em toda a trilha 21-D-A/21-D-B, o Gemini nunca devolveu uma `signature` real — `step.signature` chegou vazio em toda chamada observada. Sem assinatura, `reasoningSignatures` fica sempre ausente e nenhum `thought` step é reconstruído; o critério "nunca fabricar" (D21D.8) está funcionando como desenhado, mas isso também significa que o reenvio nunca foi de fato exercitado contra a API real. Confirmar continua sendo trabalho do usuário, condicionado à Google começar a preencher o campo de verdade.

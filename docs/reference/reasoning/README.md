...
- **21-C — orçamento sob raciocínio, faixas fixas, e o martelo do Gemini.** Implementado em 03/09/2026, verificado ao vivo no mesmo dia, três planos (revisão do advisor separou por não compartilharem código): [`21-C-A`](../../plan/active/21-C-A-orcamento-de-geracao.md) (headroom de geração reservado, calibração por média móvel — **mais um complemento na mesma sessão de verificação**: o medidor nunca exibia os `promptTokens`/`evalTokens` reais do provedor, só a estimativa de caracteres, e a razão se reaplicava sobre a história inteira a cada envio em vez de ancorar no último turno medido de verdade — D21C.12/D21C.13, legenda visível + ancoramento pós-fato, hidratado ao reabrir conversa), [`21-C-B`](../../plan/active/21-C-B-motivo-de-parada.md) (`context-exhausted`, sondagem ao vivo confirmou `done_reason:'length'`), [`21-C-C`](../../plan/active/21-C-C-faixas-fixas-de-contexto.md) (faixas fixas + campo numérico, `exposesReasoning` desliga o switch do Gemini, causa do D21A.10 fechada — § acima).

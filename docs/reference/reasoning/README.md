...
⚠️ **Retrato de ago/2026, desfeito pela execução — leia o parágrafo abaixo como história.** O array `TOOLS` com três rótulos não existe mais: o 23-D reformou o popover inteiro (o corpo virou um só, `AttachMenu.tsx` nasceu) e **apagou a linha `Documentação (MCP)`** (D23D.5) — consultar documentação virou item da lista de anexos, porque é ação do usuário que produz um anexo, não capacidade do modelo. `Raciocínio visível` continua sendo interruptor, e é a única das três que sobrou como tal. Donos: skills [`ctx-7`](../../../.claude/skills/ctx-7/SKILL.md) e [`ai`](../../../.claude/skills/ai/SKILL.md).
...
**Implementado e verificado ao vivo em [`21-D-A`](../../plan/implemented/21-D-A-interactions-api-o-parser-novo.md), 04–05/09/2026 — raciocínio real do Gemini confirmado na tela.** `gemini.ts` fala com `POST /v1beta/interactions` (`store: false`, `stream: true`), corpo em `snake_case`, `system_instruction` como string solta (não `{parts:[...]}`), `input` como lista plana de `{type: 'user_input'|'model_output', content: [...]}`. O parser lê `data: {...}` linha a linha e despacha pelo campo `event_type` do próprio JSON. **A forma real, confirmada ao vivo, divergiu da documentação em quase todo campo** (quatro tentativas foram necessárias — detalhe técnico de cada achado em `ARMADILHAS.md`, entradas "Interactions API do Gemini…"): um `thought_summary`/`thought_signature` delta embrulha o texto em `content: {text, type}`, nunca um campo `text` solto; existem **duas** formas de evento de status — `interaction.status_update` (mid-stream) carrega `status` solto no topo, `interaction.completed` (final) aninha `status`/`usage` sob `interaction: {...}`, com nomes de campo reais (`total_input_tokens`/`total_output_tokens`, nunca `usage_metadata`/`prompt_token_count`); existe também um evento `step.stop` (`{index, event_type}`) que fecha um step explicitamente. A `signature` de um `thought` step só conta se algum evento a preencheu antes do fim do stream (nunca no `step.start`, onde chega vazia) — mas sua ausência **não** esconde o texto do raciocínio, que aparece sempre (achado tardio: uma correção anterior tinha confundido "não persistir a assinatura ausente" com "não mostrar o texto").

**Guarda de contrato em dois graus (D21D.3), não um.** Grau 1 — step de tipo desconhecido (`function_call`, `file_search_call`, `code_execution_call`, ou o que a Google acrescentar), linha `data:` não-JSON (sentinela `[DONE]`, confirmado ao vivo), evento/delta não reconhecido: loga e ignora, o turno segue. Grau 2 — falta estrutural (nenhum `steps`, nenhum `model_output` com texto num turno completo): `UpstreamError` nomeada, mesmo caminho de `AppError.upstream` que já existia. Motivo de parada (D21D.1): `status: 'incomplete'` → `'context-exhausted'`; `status: 'budget_exceeded'` → erro upstream próprio, nunca colapsado no mesmo rótulo, mas sem descartar conteúdo já recebido — **nenhum dos dois confirmado ao vivo ainda** (descartado por decisão do usuário: uso real do Gemini no app é sempre 256k–1024k de janela, forçar 4k não reflete cenário nenhum real).

**Persistência e reenvio da `signature`, implementados no [`21-D-B`](../../plan/implemented/21-D-B-signature-resend-e-orcamento.md), 05/09/2026.** `reasoningPartSchema` ganhou `signature` opcional (D21D.6); `ChatMessage.reasoningSignatures` (lista, comprimento ≤ 1 hoje — `reasoningPartOf` continua "nunca mais de uma por turno") carrega o texto e a assinatura de volta para `gemini.ts`, que reconstrói o(s) `thought` step(s) na frente do `model_output` daquele turno (D21D.8/D21D.8.1); `historyCharsOf` (`core/ai/messages.ts`) soma o texto reenviado ao orçamento nos três pontos que derivam chars de `Message[]` (D21D.9). `exposesReasoning()` já incluía Gemini desde o 21-D-A (D21D.4); nada muda aqui.

✅ **Confirmado ao vivo horas depois do fechamento — e com um bug real.** A primeira `signature` de toda a trilha 21-D-A/21-D-B chegou (três sessões seguidas do usuário), o reenvio disparou de verdade no segundo turno, e a API recusou com `HTTP 400: "The 'type' parameter is required at 'input[1].summary[0]'."` — `toInteractionsInput` reconstruía `summary: [{ text }]`, sem o campo `type` que o Content-union da API exige na direção de envio (o parser de **recebimento** trata esse mesmo campo como opcional, o que escondeu a lacuna dos três testes escritos contra o próprio código). Corrigido para `summary: [{ type: 'text', text }]`, mesma forma que `content` já usa em `model_output`. Detalhe: `ARMADILHAS.md` § *`thought` step reenviado precisa de `type`*.

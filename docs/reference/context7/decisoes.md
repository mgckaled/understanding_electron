...
- **DM-18 · DECIDIDA (07/09/2026) — `fast=false`** *(juízo)*. O gargalo do projeto é janela de contexto, não latência; ordenação melhor por alguns segundos é a troca certa. ⚠️ **Deixou de ser absoluta em 12/09/2026 e foi construída no 23-J — ver as DUAS linhas de DM-18 em § *O que a execução fez*.**
...
| **DM-32** | 23-D · 23-E | **Cumprida e sem pendência de licença.** O ícone `Library` serve o popover, o cabeçalho do painel e o painel inteiro; a palavra "Context7" é o crédito, em texto. A silhueta a 16px contra `NotebookPen`/`Paperclip` continua sendo verificação do fechamento, e só é possível lá, quando o contador do 23-H existir |
...
| **DM-18** | 23-J | ⚠️ **Deixa de ser absoluta: o app passará a oferecer as duas.** A decisão original escolheu `fast=false` porque *"o gargalo do projeto é janela de contexto"* — argumento tomado em 07/09, **antes de a seleção existir**. O 23-G entregou caixa por trecho, custo exato por item e veredito de caber: a máquina que torna 25 trechos administráveis. Medido no 23-A, mesma pergunta: `fast=false` devolve **3 trechos / 383 tok**, `fast=true` devolve **25 / 3.497 tok**, pela mesma chamada de cota. O que se perde é real — sem reranqueamento a ordem é arbitrária, e escanear 25 títulos é trabalho —, então nem trocar nem ficar: uma caixa **`consulta ampla`, desligada por padrão**, na primeira tela, ao lado da cota e do estado da chave, que são as outras duas administrações da consulta. Custo: um booleano em `docs:fetch`, que já leva `libraryId`, `query` e `version`; o painel já desenha as duas formas. **Levantado pelo dono ao usar** (*"5 trechos podem não ser suficientes"*), decidido em 12/09/2026, e o corte dono é o **23-J** (era `23-L` até o remanejamento de letras de 12/09/2026) — é opção de composição, não modo de falha, e enfiá-la no 23-I repetiria o erro que a separação entre minúcias e livro-razão acabou de desfazer |
...
| **DM-31** | 23-H | ✅ **Cumprida inteira, e a única parte dela que era desenho mudou.** A linha muda de aparência na transcrição, o `histórico` lista e soma só as ativas, e o desligar chegou à tela. O que a decisão punha **só** no histórico ficou nos **dois** lugares, com um dono só (D23H.1): nenhum dos dois guarda estado. E as "três coisas em código" que ela previa viraram duas — o filtro em `partForProvider` já existia desde o 23-C, então o corte não escreveu uma linha de reenvio |
| **DM-32** | 23-H | ✅ **Verificada ao vivo, três cortes depois de proposta.** As três silhuetas a 16px são distintas (`Library` · `NotebookPen` · `Paperclip`), o que só este corte tornou possível — antes dele o terceiro contador não existia. O veredito formal segue listado para o fechamento (o **23-K**, que era `23-J` antes de 12/09/2026), mas não há o que decidir: o ícone fica |
...
| **DM-12** | 23-I | ✅ **Cumprida, e dez viraram onze — a décima primeira era uma dívida aberta, não uma situação nova.** O teto de 30 s do cliente era a **condição 1 de D23B.1**, a que sustentou o `invoke` simples contra o job (*"o teto aparece na tela, nunca em silêncio"*), e estava descumprida desde o 23-B sem que ninguém notasse, porque o teto nunca foi atingido. ⚠️ **E a coluna "ação" da tabela mudou para duas das onze:** a DM-12 escreveu *"abre Configurações"* para as situações 5 e 7, e isso não é barato — `Settings` é autocontido em duas instâncias do `App.tsx`, então um comando global exigiria erguer estado para a casca. **A ação virou a frase que nomeia Configurações** (D23I.4), que é o precedente real do repositório (`useCloudCatalog`, para Gemini e GLM). As demais nove não viraram botão próprio: o rodapé **já é** a linha de ações, então `retry` troca o rótulo do primário e `back` o remove |
| **DM-19** | 23-I | ✅ **A metade "opcional" morreu de vez; a do cofre fica inteira.** O portão está nos dois `Consultar`, e a distinção contra D23D.3 ficou escrita (D23I.5): o item `Documentação` do menu `+` continua nunca travando, porque é lá que a explicação cabe. Custo real: **zero canal, zero schema, zero linha no main** — `useCloudSecret('context7')` já compilava desde o 23-B. ⚠️ **E a sonda que a acompanhava respondeu:** anônimo e com-chave são contadores **separados** (55 contra 201 consumidos no mesmo instante), o que **não** autoriza somá-los — [`api.md`](api.md) |
| **DM-23** | 23-C | **Cumprida, e a contradição interna do desenho resolvida junto.** O que a parte persiste ficou fixado em D23C.3 |
| **DM-18** | 23-J | ✅ **Cumprida na forma nova, e o *como* acrescentou o que a decisão não podia prever.** A caixa existe, desligada por padrão, com o custo no rótulo. Três coisas se decidiram na execução: ela fica **sob o campo Pergunta e não dentro do `DocsAdmin`** como a errata escrevera, porque altera a **requisição** enquanto cota e chave descrevem a **conta** (D23J.6); é **caixa de marcação e nunca `Switch`**, porque só age quando `Consultar` é apertado (D23J.7); e `broad` **volta a `false`** na consulta seguinte — *"desligada por padrão"* é por consulta, não por sessão (D23J.5). ⚠️ **E uma peça que nenhuma decisão tinha: D23J.4.** Sem a amplitude na chave do memo de sessão, ligar a caixa devolveria a resposta estreita já paga, de graça — a caixa leria como se não fizesse nada. Verificado ao vivo: `Trechos (25)` contra os ~5 da mesma pergunta |
| **DM-28** | 23-J | ✅ **A tabela dela fecha com os dois elementos que a execução acrescentou.** O 23-F resolveu a **descrição**, o 23-G achou que o **título** também é markdown e nomeou o conserto, e o 23-J o fez: variante `inline` do `MarkdownMessage`, `span` em vez de `p`, **sem declarar `font-size`** — herdar era o ponto, porque o título vive num `<button>` que trunca. Vale nos **quatro** lugares que o desenham, não só no painel: uma string, um dono, três telas que não podem divergir |
| **DM-16 · DM-25** | 23-J | **Cumpridas desde o 23-G, e uma premissa vizinha caiu agora.** D23G.5 recusara `marcar todos` com dois argumentos, e o caminho amplo derruba o primeiro: *"o caso medido é de 5 trechos"* vira 25. O segundo — a caixa de estado misto do APG, que exige `indeterminate` por `ref` — **nunca se aplicou a dois botões de texto**, que é o que nasceu (D23J.8). Escopados às chaves da aba Trechos, porque notas seguem em ~5 mesmo no caminho amplo |

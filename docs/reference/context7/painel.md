...
> ⛔ **Documento consumido — leitura histórica, não fonte de regra** (arco 23 fechado em 13/09/2026): a interface **como ela ficou** é [`painel.md`](../../../.claude/skills/ctx-7/painel.md) da skill; aqui fica o desenho de antes da tela existir, com o aviso de cada divergência. **Não `Read` inteiro** — `Grep` pelo termo.

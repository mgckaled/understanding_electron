...
- **Não existe endpoint de cota** — nenhuma rota reporta o consumo sem gastar uma chamada; o painel do site é o único outro lugar. Conferido contra a documentação oficial em 12/09/2026 (23-I), e é o que torna `sem consulta nesta sessão` inevitável na primeira tela: o número só pode vir do header da última resposta.
- **Os nomes dos headers estão documentados**: `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset`, e `Retry-After` no 429 ([API Guide](https://context7.com/docs/api-guide)). O cliente os lê em minúsculas e `Headers.get` é insensível a caixa — nada a corrigir. O formato da chave é `ctx7sk-…` (o `ctx7op-` é de conta *enterprise*).
- ⚠️ **Levantado, NUNCA sondado:** o regime anônimo é descrito em fonte secundária como **um pool global compartilhado** (*"unauthenticated requests share a global anonymous rate limit pool"*, DeepWiki sobre `upstash/context7`, lido em 12/09/2026). Se verdade, a cota anônima não é sua — esgota pelo tráfego de terceiros. **Procedência fraca: não citar como fato.** É a segunda coisa a observar quando a sonda dos dois regimes rodar.

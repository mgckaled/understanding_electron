...
✅ **Decidido, e a tensão já está documentada em dois lugares** — [`ESCOPO.md`](../ESCOPO.md#o-gate-de-capacidade-é-correção-não-cortesia) e [`cloud-optin.md`](models/cloud-optin.md#gemini-25-flash): o `gemini-2.5-flash` é o único candidato de nuvem com `vision` nas capacidades, e o nível 3 (ESCOPO.md) bloqueia justamente anexo de imagem/documento na nuvem, sem exceção por provedor. Um usuário vai ver um modelo que **declara** poder ver imagem, com o anexo recusado mesmo assim.

Este guia não resolve a tensão — ela já está registrada como sem solução em ambos os documentos donos. O que cabe aqui é nomear a decisão que a fatia 3 precisa tomar explicitamente, porque é fácil resolver por acidente em vez de por decisão: ocultar `vision` da lista de capacidades exibida quando o modelo é de nuvem (esconde a causa, evita a pergunta do usuário) versus mostrar a capacidade e recusar o anexo com uma dica clara do motivo (mais honesto, exige texto de interface pensado). A regra do nível 3 já existe — o que falta é só a decisão de como a UI se comporta em cima dela.

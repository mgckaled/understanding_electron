...
| [Web Fetch, MCP e Thinking Mode](web-fetch-mcp-thinking/README.md) | ago/2026 | ⛔ consumido **em 14/09/2026** | Guia gerado fora deste repositório, propondo três capacidades de chat via *tool calling* do Ollama. **As três foram consumidas, e nenhuma pelo caminho que ele previa:** a Feature 3 (Thinking) pelo arco 21, sem *tool calling* em provedor nenhum; a Feature 2 (MCP) pelo arco 23, que trocou MCP por **REST** (DM-0) — dona hoje é a skill [`ctx-7`](../../.claude/skills/ctx-7/SKILL.md); e a Feature 1 (busca web) **substituída pelo levantamento próprio do arco 22**, na linha abaixo. ⚠️ A premissa comum às três — *tool calling* como espinha dorsal — foi derrubada **três** vezes agora. Leitura histórica: o valor que sobra é o registro de uma premissa que não sobreviveu ao contato com nenhum dos três arcos |
...
| [Busca web — levantamento do arco 22](web-search/README.md) | 14/09/2026 | ✅ vivo, **nada decidido** | **Material de entrada do arco 22**, substituindo a Feature 1 do guia antigo. O achado que reorganiza o pilar: **são duas operações, não uma** — `web_search` (uma pergunta sai da máquina, ~2.100 tok por resultado) e `web_fetch` (só o endereço sai, ~1.550 tok), e a conta da Ollama já as cobra em linhas separadas. ⚠️ O [`ESCOPO.md`](../ESCOPO.md) define o pilar como **fetch**; o interruptor que já está na tela se chama **busca**. ⚠️ **Não há header de cota — nenhum** —, então o `docs:quota` do Context7 não tem equivalente possível. Traz a aritmética contra o `qwen3.5` (o default `max_results: 5` da API consome **metade** do orçamento de prompt desta máquina) e um levantamento de segurança cujo enquadramento é a **trifecta letal**: o arco 22 é precisamente o que promove a perna *conteúdo não confiável* de parcial a plena, então a pergunta do arco não é *como filtrar injeção* — é **qual perna fica de fora**. ✅ A exfiltração de **zero** clique (imagem markdown) já está bloqueada em duas camadas; ❌ a de **um** clique (link com texto enganoso) **está aberta hoje**, e o conserto não depende deste arco; ❌ caractere **Unicode invisível** atravessa o painel, derrubando a premissa de que "a pessoa vê o que entra no contexto"; ❌ e `checkExternalUrl` **não** é guarda de SSRF, apesar de parecer. **29 decisões `DW-<n>` levantadas e nenhuma fechada** — a proposta de cortes que fecha o documento é **condicional a elas**: só os dois primeiros (os dois furos acima) valem qualquer que seja a resposta. Quatro anexos: [`api.md`](web-search/api.md), [`seguranca.md`](web-search/seguranca.md), [`painel.md`](web-search/painel.md), [`decisoes.md`](web-search/decisoes.md) |

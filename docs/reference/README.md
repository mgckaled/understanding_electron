...
| [Ollama Cloud — sondagem da API](ollama-cloud/README.md) | 13/09/2026 | ✅ vivo | Os seis modelos da faixa gratuita, medidos contra o serviço real em 27 testes: capacidades, contexto, velocidade (até **315 tok/s**, ~8× o melhor local), cota e o que ela conta de fato. O adaptador `ollama.ts` serve quase inteiro; **`format` não existe na nuvem** — e a sondagem expôs que `ai:propose` já dependia disso em silêncio nos três provedores. Traz também duas capacidades que o daemon local não tem — **busca web e extração de página** —, que tocam uma decisão em aberto do `ESCOPO.md`. ⚠️ *Tool calling* funciona em **quatro dos seis** (`gpt-oss:20b` devolve HTTP 400 sempre), e a bateria de qualidade mostrou que **velocidade não prediz fidelidade**. Material de entrada da trilha **N-3**. Quatro anexos: [`api.md`](ollama-cloud/api.md), [`desempenho.md`](ollama-cloud/desempenho.md), [`capacidades.md`](ollama-cloud/capacidades.md), [`modelos.md`](ollama-cloud/modelos.md) |

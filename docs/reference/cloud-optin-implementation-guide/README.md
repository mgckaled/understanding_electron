...
> ⛔ **Documento consumido — leitura histórica, não fonte de regra.** Era o levantamento prévio da fatia 3, escrito antes de existir plano; a fatia foi implementada pela trilha N-1 (N-1-A segredo, N-1-B GLM, N-1-C Gemini, ago/2026). O que valeu virou código e decisão registrada em [`HISTORY.md`](../../HISTORY.md); o que restou dele é a **Peça F** (provedor terceirizado), que abre a trilha N-2 e ainda não tem plano. **Não `Read` inteiro** — `Grep` pela peça ou pelo provedor.

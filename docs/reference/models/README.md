...
| [`cloud-optin.md`](cloud-optin.md) | Ficha técnica de **nove** candidatos de nuvem (recontados em 14/09/2026) — **cinco integrados**: três desde N-1-B/N-1-C (`gemini-3.5-flash-lite`, `gemini-3.7-flash`, `glm-4.7-flash`) e dois na N-3 (`gemma4:31b`, `gpt-oss:120b`, os únicos com catálogo **sondado** em vez de escrito à mão) — e quatro elegíveis via provedor terceirizado (Kimi K2 e Qwen3 32B via Groq, DeepSeek-V3.2 via SambaNova, GPT-OSS-120B via Cerebras), ainda fora do app — trilha N-2, que ganhou escopo declarado no fechamento da N-3 |
...
**12 entradas** no `/api/tags` em 13/09/2026 (eram 13 em 18/08), das quais 5 são variantes `-custom` por Modelfile, com os mesmos pesos e teto das originais. ⚠️ **`qwen3:4b` foi removido da frota pelo dono em 13/09/2026** — era o cache mais caro (152,6 KB/token, teto real de 35k mesmo com 8,5 GiB livres) e a dupla `qwen3.5` o domina em capacidade e custo. As distintas:
...
| `qwen3.5:2b` | 2,55 GB | 262.144 | **14.500 B/tok — medido** | `completion`, `vision`, `tools`, `thinking` | instalado fora do app (visto em 01/09/2026), medido `/api/show` na sessão do R-6 (03/09/2026). **Único da frota com as quatro capacidades juntas** — foi o primeiro a ter `vision` e `tools` juntas — o que abriu a **F-6** e a fechou: medido em 14/09/2026, a coexistência não quebra nada, porque o gate sempre foi por **anexo**, não sobre o modelo inteiro. É o modelo local mais usado na prática (Observatório, painel Desempenho). ⚠️ `head_count_kv` vem `null` porque **`qwen35` é híbrido Mamba** (bloco `ssm.*` completo), como o `granite4` — mas **este está na frota**. O KV/token ao lado foi **medido** em 13/09/2026 pela inclinação do residente entre `num_ctx` 2048 e 16384, não calculado: [`ARMADILHAS.md`](../../ARMADILHAS.md) § `qwen3.5:2b` e `qwen3.5:4b` também têm `attention.head_count_kv: null`. Teto real nesta máquina a 5,65 GiB livres: **168k**, não os 262k que o app exibe hoje |
| `qwen3.5:4b` | 3,16 GB | 262.144 | **36.985 B/tok — medido** | `completion`, `vision`, `tools`, `thinking` | instalado fora do app, medido `/api/show` em **13/09/2026** (sessão de refinamento da N-3, ao notar que o `/api/tags` local trazia um modelo que nenhum documento conhecia). 4,7 B de parâmetros, `Q4_K_M`, arquitetura `qwen35`, `head_count` 16. **O irmão maior do `qwen3.5:2b`: mesmas quatro capacidades, mesmo teto, e ainda cabe nesta máquina** — é o preferido quando a RAM permite. ⚠️ `head_count_kv` vem `null` **também aqui** — a causa é arquitetura: `qwen35` publica bloco `ssm.*` completo e é **híbrido Mamba**, com `full_attention_interval = 4`, então **só 8 das 32 camadas crescem** com o contexto. O KV/token ao lado foi **medido**, e derrubou a hipótese da F-6 (usar `head_count`), que preveria 347.341 B/token — **9,4× para cima**. Teto real a 5,65 GiB livres: **48k**, não os 262k exibidos hoje. Publica também um bloco `qwen35.vision.*` paralelo, com `.block_count` próprio — o caso que `readInfo` trata por caminho, não por sufixo (D15.8) |
...
⚠️ **Esta tabela data de 18/08/2026 e está incompleta** — `qwen3.5:2b` e `qwen3.5:4b` foram instalados fora do app depois dessa medição e adicionados pontualmente (R-6 e a sessão de 13/09/2026), porque são os dois mais usados. **O `/api/tags` local devolveu 12 entradas em 13/09/2026**, depois da remoção do `qwen3:4b`; os pesos dos demais **não** foram remedidos. Um reinventário completo da frota segue sendo trabalho separado, ainda não agendado.

⚠️ **E o modo como o `qwen3.5:4b` foi descoberto é o argumento para agendá-lo.** Ele apareceu num `curl` ao `/api/tags` feito de passagem, durante uma sessão sobre **nuvem** — não por alguém conferir a frota. Nenhum documento do repositório o citava, e ele já era o modelo preferido de quem usa o app. **A frota muda por fora do repositório, em silêncio**, e nada aqui observa isso: o Observatório vê o que o app usou, não o que está instalado.

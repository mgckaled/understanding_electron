...
| `qwen3.5:4b` | 3,16 GB | 262.144 | **não custeável — bug** | `completion`, `vision`, `tools`, `thinking` | instalado fora do app, medido `/api/show` em **13/09/2026** (sessão de refinamento da N-3, ao notar que o `/api/tags` local trazia um modelo que nenhum documento conhecia). 4,7 B de parâmetros, `Q4_K_M`, arquitetura `qwen35`, `head_count` 16. **O irmão maior do `qwen3.5:2b`: mesmas quatro capacidades, mesmo teto, e ainda cabe nesta máquina** — é o preferido quando a RAM permite. ⚠️ `attention.head_count_kv` vem `null` **também aqui**, então o bug de custeio deixa de ser caso isolado e passa a ser propriedade da família `qwen35`: a hipótese da **F-6** (usar `head_count` no lugar) agora vale para dois modelos. Publica um bloco `qwen35.vision.*` paralelo, com `.block_count` próprio — o caso que `readInfo` trata por caminho, não por sufixo (D15.8) |

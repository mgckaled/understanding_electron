...
> ⛔ **Documento consumido — leitura histórica, não fonte de regra.** A trilha N-3 fechou em 14/09/2026, com cinco cortes entregues e verificados ao vivo. O que ainda amarra código passou a ter **dois** donos vivos, e essa divisão é a diferença contra o arco 23: a **regra que decide a primeira linha** está na skill [`ai`](../../../.claude/skills/ai/SKILL.md) — `attention`/`sizeBytes` forçados, o nível exigido pelo `gpt-oss`, a armadilha `-cloud`, disponibilidade por `hasKey()` apesar de o ping funcionar, `total_duration` como única duração da nuvem —, e a **ficha dos dois modelos** está em [`reference/models/cloud-optin.md`](../models/cloud-optin.md), que fica **vivo**. **Não `Read` inteiro** — `Grep` pela sigla `DNC-<n>` ou pelo assunto, se precisar do que sobra aqui, que é o *como chegamos lá*: as 82 requisições de sondagem, a série de cota, a bateria de qualidade que escolheu dois dos seis, e as quatro rejeições com motivo.
>
> **Data:** sondagem e refinamento em 13/09/2026 · **Estado:** ⛔ consumido em 14/09/2026 · **Natureza:** medição, mais o recorte que a trilha N-3 executou.
>
...
Arquivos separados, estilo 18-A — nunca passos dentro de um arquivo só, e **nunca subcorte** (a forma `23-A-1` do arco 21 não se pagou: o filho carrega o contexto do pai junto, o que anula o ganho de fatiar). **Corte que crescer demais vira dois irmãos**, ambos no mesmo nível, cada um com o próprio arquivo e o próprio diário. A primeira letra livre é `F`. ⚠️ **E ela quase foi `G`:** o `N-3-D` chegou a acrescentar um sexto corte para a escolha de esforço (`DN3D.8`), desfeito no `N-3-E` quando a medição mostrou que nenhum modelo da frota local honra nível — o assunto virou escopo da trilha `N-2` (`DN3E.2`).
...
| **N-3-D** | ✅ **entregue em 14/09/2026.** O nível sobe ao `ChatFn` e o handler o preenche pelo nome de família (`DN3D.1`/`DN3D.2`); `'low'` vai **nos dois estados do interruptor**, porque a família não tem desligado (`DN3D.3`); `gpt-oss:120b` na lista fixa; `total_duration` vira **coluna própria** no painel, com migração `v5` (`DN3D.5`/`DN3D.6`) | C |
| **N-3-E** | ✅ **entregue em 14/09/2026.** O piso do `gpt-oss` vai para `medium` — o default do próprio modelo, porque esforço compra qualidade de **resposta** e não só tamanho de rastro (`DN3E.1`) —, a escolha de esforço vira escopo da `N-2` (`DN3E.2`), e o fechamento. Verificação ao vivo, e o que sobrevive se divide em **três** destinos: regra que decide a primeira linha vai para a skill `ai`; ficha dos dois modelos de nuvem vai para `reference/models/`, que fica **vivo**; a narrativa da sondagem fica aqui, marcada `⛔ consumido`. Mais `ESCOPO.md`, pasta e apontadores (`DNC-30`) | todos |
...
⚠️ **Não existe `N-4` nem `N-5` nesta trilha.** Dois candidatos foram avaliados e **recusados como trilha**, cada um com o seu registro: a saída estruturada por ferramenta forçada vira **gatilho** no [`ROADMAP § 2`](../../ROADMAP.md) em vez de corte reservado (`DNC-19`), e a **busca web** é do pilar próprio dela, não deste provedor (`DNC-21`). Reservar letra para trabalho que talvez nunca aconteça é o ponto de extensão especulativo que `DNC-1` recusa. ⚠️ **E a regra se provou no próprio fechamento:** o `N-3-D` reservou uma letra para a escolha de esforço, e a medição do dia seguinte mostrou que a escada seria de **um** modelo — o trabalho foi para a trilha `N-2`, onde os provedores têm esforço nativo, e a letra voltou. **A primeira letra livre para um irmão é `F`.**
...
|---|---|---|
| **A** | a guarda reprova sob sabotagem; e o teto dos dois `qwen3.5` **ao vivo**, não só em teste | Um teste de "não persiste raciocínio" pode nascer vacuoso se o campo nem for renderizado nesse estado — já aconteceu neste projeto (skill [`testing`](../../../.claude/skills/testing/SKILL.md)). Prove o **estado final** que o defeito inverteria. E `DNC-31` muda o que o popover e o `ContextControl` **mostram**. ⚠️ **ERRATA (`DN3A.10`, executado em 13/09/2026): os dois números desta frase estavam mortos.** `até 32k` é resíduo da **primeira** versão de `DNC-31`, a do fallback em `DEFAULT_NUM_CTX`, reescrita e reprovada na mesma sessão — o esperado é o teto **real**, que se move com a RAM livre (medido: 26k–53k no `4b`, 112k–183k no `2b`). E `qwen3:4b` **não está instalado**; o contraste real desta máquina é `qwen2.5:7b`. O que reprova é continuar em **262.144** |
...
| **D** | ~~**quanto `think: 'low'` reduz de fato**~~ ✅ **medido em 14/09/2026, e não pelo caminho previsto.** O rastro do `gpt-oss:120b` com `'low'` veio em **uma linha** contra os 332 tokens do nível padrão. ⚠️ **O caminho previsto — comparar a geração de dois turnos — era inválido por construção:** o prompt cresce entre turnos, e o contador `Geração` soma raciocínio **e** resposta num número só (o app não tem contador separado; é pergunta do `O-9`) | — |

...
**SQL gerado por modelo roda com o motor restringido, não com o texto inspecionado.** O motor é travado — sem acesso externo, com as pastas que pode ler declaradas — antes de qualquer consulta gerada, e a trava é definitiva, não uma que liga e desliga. A garantia é do motor; uma expressão regular tentando adivinhar intenção em SQL não é defesa. Quais comandos, em que ordem e por que a ordem não é livre: skill [`data`](../.claude/skills/data/SKILL.md).
...
- **Exportar em `.xml`** — não custaria biblioteca nenhuma, mas custaria uma decisão de esquema que nada pede: exportar o texto que o modelo escreveu, ou serializar a conversa num envelope? Recusar as duas é mais barato que escolher uma, e `.txt` cobre o caso real.

...
3. Ao concluir, o arquivo **move** para [`plan/implemented/`](plan/implemented/) e ganha **uma entrada curta** em [`HISTORY.md`](HISTORY.md), com link — e **toda decisão com sigla própria** (`D<id>.<n>`) entra em [`DECISOES.md`](DECISOES.md), uma linha cada — **o formato em que ela foi escrita não importa**, seja heading (`### D<id> — …`), parágrafo em negrito ou item de lista. Um plano cujas decisões são prosa corrida **sem sigla** ganha uma linha só, com a descrição igual ao título do marco recém-escrito em `HISTORY.md`. **Se essa entrada for a 11ª**, a mais antiga desce para [`HISTORY-archive.md`](HISTORY-archive.md) na mesma edição — ver [régua de tamanho de documento](#régua-de-tamanho-de-documento).
...
|---|---|---|
| `HISTORY.md` | **10 marcos** | a mais antiga desce para `HISTORY-archive.md`, na mesma edição. **Só marcos** — decisão arquitetural desce junto |
...
| referência (`reference/`) | — | uma pasta por referência, `<slug>/README.md`; anexo entra na pasta, nunca solto |
...
| Consulta de documentação (Context7) — REST e nunca MCP, as três camadas, memo de sessão, cota e chave, a pergunta que sai da máquina | skill `ctx-7`                                                       | apontam — o guia do arco em `reference/context7/` é `⛔ consumido` |
| Busca web — ainda sem plano. Raciocínio **não** é ferramenta: é capacidade de modelo, dona a skill `ai` | [`reference/web-fetch-mcp-thinking/README.md`](reference/web-fetch-mcp-thinking/README.md) | apontam — só a Feature 1 daquele guia segue viva |

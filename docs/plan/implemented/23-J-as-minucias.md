...
**D23J.11 — a linha na transcrição conta os descartados em vez de listá-los.** Decidida **depois** do plano, ao ver a tela real do caminho amplo: 7 trechos enviados e **18 riscados abaixo deles**, que é a lista de descartados enterrando a de enviados. O desenho de [`painel.md`](../../reference/context7/painel.md) previa o riscado, e estava certo para o caminho estreito — cinco trechos com um descartado. **A premissa que caiu é a mesma de D23J.8:** o que funcionava a cinco não funciona a vinte e cinco.

O registro **não some**, e é isso que autoriza a remoção: a parte persistida continua guardando título e custo de cada omitido (D23C.3), e a releitura tem **aba própria** para eles desde D23H.9 — a linha passa a dizer `N trechos não enviados — no painel`, que é uma linha em vez de dezoito e mantém a escolha visível na transcrição, que era o ponto do desenho original.
...
|---|---|---|
| 12/09/2026 | Plano escrito a partir do guia do arco 23 (os quatro arquivos, na íntegra) e dos **nove** diários; D23J.1–D23J.10 fixadas | Três forquilhas resolvidas pelo dono: a caixa em linha própria sob Pergunta (não dentro do `DocsAdmin`, como a errata de DM-18 escrevera), `marcar/desmarcar todos` contra as vinte desmarcações à mão, e a variante inline nos **quatro** lugares que desenham o título. **Um achado que não estava em documento nenhum: D23J.4** — a chave do memo de sessão não inclui a amplitude, então ligar a caixa devolveria a resposta estreita já paga, de graça, e a caixa leria como se não fizesse nada. Pesquisa externa obrigatória: `disallowedElements`+`unwrapDisallowed` é a API documentada do `react-markdown` para inline (Context7), e `fast=true` é **mais rápido** por pular o reranqueamento deles — o gatilho do `invoke`-vs-job não corre risco aqui |
| 12/09/2026 | Passo 1: a variante inline, e uma regressão de desempenho que o portão pegou | ⚠️ **O `commit_gate` reprovou o primeiro commit, e estava certo:** dois testes de `contextBudget.test.tsx` estouraram o teto de 5 s **sob a suíte inteira**, passando isolados. Não era lógica — era custo. Medido em vez de suposto: o arquivo foi de **18,71 s** (sem a variante) para **21,17 s** (com ela), e os dois já viviam a ~2,2 s, ou seja a ~2,3× do teto sob contenção. O conserto **não** foi desfazer a variante nem subir o timeout: `MarkdownMessage` virou `memo`, porque o custo é o **parse** e um título vive em árvores que rerenderizam a cada tecla do composer — voltou a **19,2 s**. É a régua da skill `design-system` sendo seguida à risca: *se o custo do `react-markdown` numa legenda incomodar, a resposta é medir, não escrever um parser* |
| 12/09/2026 | Passo 2: `broad` atravessa as quatro camadas, e a caixa nasce | Vermelho provado um a um, cada um caindo num teste nomeado: o ternário invertido derruba os **dois** lados no nível 1, a chave antiga do memo derruba o teste da amplitude no nível 3 — com `amplitude` **mantido em uso** na sabotagem, para o `typecheck` não reprovar antes e provar o compilador em vez do teste —, e as duas sabotagens do provider caem cada uma no seu teste de nível 2. ⚠️ **Dois rótulos meus prometiam mais do que o teste afirmava** (`"and is absent from it when off"` sem a asserção de ausência; `"refuses the attach"` com `toBeEnabled`): corrigidos os nomes, não inventadas as asserções |
| 12/09/2026 | Passo 3: `marcar todos` / `desmarcar todos` | ⚠️ **`Tests no tests` outra vez, e por causa nova:** um apóstrofo dentro do nome de um teste (`this tab's keys`) quebrou a string e o esbuild recusou o arquivo. A entrada do `ARMADILHAS.md` nasceu no 23-F com `\&` escapado pelo shell; a família é a mesma — **a suíte não correu**, e ler isso como vermelho teria "confirmado" uma sabotagem que nunca foi exercitada. Vermelho verdadeiro provado depois nos três: escopo largo demais (chaves de trechos **e** notas) derruba os dois testes de escopo, e o par sobrevivendo ao `Anexar` derruba o terceiro |
| 12/09/2026 | Suíte remedida, à espera do passo 4 | `pnpm test`: **158 arquivos / 1569 testes, 137,57 s** (era 157/1556 no fecho do 23-I). Canais: **54**, nenhum criado — `broad` é campo de um schema que já existia. Falta o passo 4, que é do dono |
| 13/09/2026 | Passo 4: verificação ao vivo pelo dono — **tudo funcionando**, e um print prova quatro coisas de uma vez | O print da tela ampla carrega, junto: o chip `onMutate` no título (*Optimistic Cache Update with Rollback using `onMutate`*, o mesmo trecho que o 23-G viu sair cru), **`Trechos (25)`** contra os ~5 do caminho estreito — que é **D23J.4 na tela**, porque a mesma biblioteca e a mesma pergunta devolveram resposta outra em vez da já memoizada —, o par `marcar todos · desmarcar todos`, e o rodapé em `2 de 25 · ~449 tok · cabe (janela 32.768, 99% livre)` com os não escolhidos riscados. ⚠️ **O item 5 do roteiro não foi exercitado, e o erro é meu, de dimensionamento:** 3.497 tok numa janela de 32.768 cabem com folga, então `não cabe` não tinha como aparecer — **é o mesmo erro que o roteiro do 23-G cometeu no item 5**, e duas vezes já é padrão: um roteiro que promete um limite precisa dizer **em que janela** ele morde |
| 13/09/2026 | D23J.11, pedida pelo dono ao ver a tela ampla | A linha da transcrição listava os **18 descartados** abaixo dos 7 enviados — a lista do que não foi enterrando a do que foi. Trocada por uma contagem de uma linha (`N trechos não enviados — no painel`). ⚠️ **É a segunda vez no corte que uma decisão do arco se mostra calibrada para cinco trechos e não para vinte e cinco** — a primeira foi D23G.5, que D23J.8 reabriu. O caminho amplo não acrescentou só volume: ele **audita as decisões de interface tomadas quando o volume era pequeno**, e essa é a lição que sobrevive ao corte. Vermelho provado devolvendo a lista ao lado da contagem. ⚠️ Um susto no meio: um teste vermelho que não era meu — a máquina estava respondendo com um modelo pesado, e a contenção derrubou um teste vizinho por `Unable to find role=button`. Mesma família do timeout do passo 1 |
| 13/09/2026 | Fechamento | Errata devolvida aos três documentos do guia, `ROADMAP § 4` (a pendência do título **sai**, a dos `aria-expanded` passa a adiada com data), duas skills atualizadas, `HISTORY.md` com a 11ª entrada empurrando a mais antiga para o arquivo, e `CLAUDE.md` remedido |

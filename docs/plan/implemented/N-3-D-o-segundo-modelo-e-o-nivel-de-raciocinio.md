...
> Quarto corte da trilha N-3. Material de entrada: os seis arquivos de [`docs/reference/ollama-cloud/`](../../reference/ollama-cloud/README.md) lidos na íntegra, os diários dos [`N-3-A`](N-3-A-a-trava-antes-da-porta.md), [`N-3-B`](N-3-B-o-popover-em-uma-linha.md) e [`N-3-C`](N-3-C-o-servico-e-o-catalogo-sondado.md), as sete skills, e refinamento externo obrigatório (Context7 na doc oficial do Ollama sobre `think`; busca web sobre `thinking_level` do Gemini). Siglas nascem como `DN3D.n`.
...
|---|---|---|
| 14/09/2026 | Passo 5: verificação ao vivo pelo dono — **nove de nove** — e fechamento | **O item 5 do roteiro era inválido por construção, e o `ok` nele não vale como medição.** Ele pedia comparar a geração de dois turnos: mas o prompt cresce entre turnos (3.550 → 5.741 na conversa real, com 27 trechos do Context7 anexados) e o contador `Geração` soma raciocínio **e** resposta num número só — o app não tem contador separado, que é pergunta do `O-9`, e o próprio `chatTiming.ts` já registrava isso. **É a terceira vez que um roteiro deste projeto promete um número de uma grandeza movida por outra causa** (23-G, 23-J, N-3-A). ✅ **O que mediu `DN3D.7` foi o print:** o raciocínio do `gpt-oss:120b` com `'low'` veio em **uma linha** — *"Need to provide advanced DataFrame example using snippets."* — contra os 332 tokens do nível padrão. **Dois achados da verificação que viraram ressalva permanente:** (a) o `gemma4:31b` mostra `—` na coluna nova porque suas três linhas são do `N-3-C`, anteriores à migração — `NULL` é a resposta verdadeira (`DN3D.6`); (b) daí decorre que comparar colunas do painel **só vale num balde cujas linhas todas carreguem o total** — no `gpt-oss:120b` vale (1,5 s + 9,5 s contra 9,9 s → ~1,1 s de rede), no `qwen3.5:2b` os 162,5 s de Rede+Prefill são média de 23 linhas e os 116,5 s de total, média só das novas. **Quarta leitura de cota** (pedida pelo dono): 82 requisições = **2,2 %**, e o lote de +6 custou **+0,3 pp**, mais que o de +34 da sondagem — `DNC-16` reforçado, não refinado. Fechamento: `HISTORY.md` com a 11ª entrada empurrando a `23-E` para o arquivo, oito linhas em `DECISOES.md` (601 → **609**, remedidas), quatro trechos da skill `ai`, `ROADMAP` com a renumeração, e três trechos de `reference/ollama-cloud/` que a renumeração tornou falsos |
| 14/09/2026 | Passos 1–4 implementados, um commit cada | **Duas sabotagens, as que o plano nomeou, e as duas discriminaram com o nome do teste na lista.** (1) Mandar `false` com o interruptor desligado derrubou **só** o teste do caso desligado, com os quatro testes de `think` vizinhos verdes. (2) Devolver `0` no lugar de ausente derrubou **cinco**, o de ausência entre eles — e os outros quatro mostram por que ausência-não-é-zero é regra de casa: todo teste de igualdade exata do adaptador viu o campo aparecer. **Um teste reprovou sozinho antes de eu tocá-lo:** a contagem de requisições do catálogo de nuvem (2 → 3) ao acrescentar o segundo modelo — era o discriminante de `DN3C.4` fazendo o trabalho dele um corte depois. ⚠️ **Um erro meu no passo 1:** esqueci o import de `requiredThinkLevel`, e o `ReferenceError` caiu no `catch` do handler virando `AppError.unavailable` — seis testes de chat vermelhos, nenhum deles apontando a causa. É a forma que um handler com `try/catch` largo dá a todo defeito de programação dentro dele |

...
|---|---|---|---|
| 18/08/2026 | — | plano escrito, ainda não executado | Sessão dedicada a desenhar o arco 18 (DuckDB) inteiro em sub-planos ≤7 passos, para não repetir o custo do 17 (duas sessões, perda de contexto de auditoria por auto-compactação). Duas incertezas técnicas resolvidas por pesquisa no Context7 antes de escrever: entrypoint do `utilityProcess` via `rollupOptions.input` multi-entrada (D18A.1) e a ordem load-antes-de-travar para a futura extensão do Excel do 18-F (D18A.3) — DuckDB aceita `LOAD` de caminho local explícito, sem tocar `autoinstall_known_extensions`. `memory_limit` remedido nesta máquina, não copiado do `ESCOPO.md`: 7 GB livres de 15,81 GB totais, sem modelo Ollama residente (D18A.4) — abaixo do "~9 GB só com Electron" do `CLAUDE.md`, mais perto do "~7,5 GB só com VS Code". Revisão do advisor (Opus) apontou quatro correções, todas aplicadas: o `.node` do DuckDB mora num pacote por plataforma (`@duckdb/node-bindings-win32-x64`), não no pacote que `pnpm add` instala — o glob do `asarUnpack` (passo 6) se escreve só depois de observar o `node_modules` real, nunca chutado; o nome do arquivo de saída do worker (passo 3) é observado, não escrito de antemão; o passo 4 ganhou prova ao vivo de que a trava é real (consulta fora de `allowed_directories` falha, `SET` pós-lock é rejeitado), não só que a lista de comandos está ordenada; `apache-arrow` saiu do passo 1 — só entra no 18-B, onde é exercitado. |
| 18/08/2026 | 1-7 | **concluído** — os 7 passos, um commit cada | Executado na sessão seguinte, plano inteiro numa sessão só. Advisor (Opus) consultado duas vezes: antes do passo 1 (achado inicial refutado por sonda isolada em Node puro — `enable_external_access = false` + `allowed_directories` juntos restringem como esperado; a leitura da doc, ambígua, tinha sugerido o contrário) e ao final, antes do fechamento — chamada que deveria ter acontecido antes do `build:win` (passo 6) e só aconteceu depois, por descuido; achou a verificação do `smartUnpack` incompleta (arquivo apagado antes de conferir `asar list`), corrigida com um build isolado adicional. Duas correções reais à ordem da D18A.3, achadas só ao vivo, nunca pela doc: `allowed_directories` e `temp_directory` têm que ser setados **antes** de `enable_external_access = false`, não depois — DuckDB rejeita mudar os dois uma vez que o acesso externo já está desligado. `memory_limit` remedido de novo no dia da execução (não só no dia do desenho): 5,54 GB livres de 15,81 GB, sem Ollama residente — mais apertado que os 7 GB do desenho, `2GB` seguiu sendo folgado. `shamefullyHoist: false` expôs uma dependência fantasma (`@types/hast`, hoisted sem estar declarada) — corrigida com `pnpm add -D`. `stdio: 'inherit'` do `utilityProcess` não repassou output do worker pelo Windows/electron-vite — trocado por `'pipe'` manual, e foi essa troca que revelou os dois erros de ordem (sem ela, pareciam hang). `asarUnpack` explícito acabou não sendo estritamente necessário — `smartUnpack: true` (default do electron-builder) já desempacota `.node` sozinho, sabotado e confirmado; mantido mesmo assim como documentação+defesa. Empacotamento bateu no `EBUSY` já registrado no `ROADMAP § 4` (Defender, provável) — contornado com `-c.directories.output=<dir>`, sem tocar `dist/` de verdade. Cinco documentos atualizados no fechamento: `ESCOPO.md` (ordem dos `SET`, `memory_limit`/`temp_directory` reais), `HISTORY.md` (marco + 3 armadilhas), `study/05-proximos-passos.md` (corrige a premissa do `asarUnpack` obrigatório), `ROADMAP.md` (gatilho do hoist fechado, 18 vira 18-A/18-B na sequência). `probeDuckdbWorker()` roda a cada boot, inclusive empacotado — scaffolding deliberado (D18A.5); 18-B remove ao trocar pelo canal real. |

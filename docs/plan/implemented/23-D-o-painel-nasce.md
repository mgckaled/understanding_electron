...
| 08/09/2026 | Passo 2: o terceiro inquilino, e o primeiro teste vacuoso | **O teste de navegação passou com o `release()` removido.** Afirmava a ausência do painel, e ele some sozinho quando a composição deixa de resolver — o `release` não mudava nada que a asserção olhasse. O que vale é o estado final da reabertura: sem ele a região continua marcada como nossa, o clique seguinte vira fechamento, e o painel aparece **um fade antes de sumir** — por isso a asserção é `data-closing`, não presença. `test/renderer-providers.tsx` acompanhou o `App.tsx` na mesma edição |
| 08/09/2026 | Passo 3: o gatilho, e a reforma que o achado 1 obrigou | O corpo do popover virou um só: detalhe do anexo acima quando existe, lista sempre. `AttachButton` foi de **318 para 240** linhas, `AttachMenu` nasceu com **100**. O painel é aberto por callback do `Composer`, não por `useDocs` dentro do `AttachButton` — os testes dele montam o componente sozinho, sem a pilha de providers. Vermelho provado devolvendo `Documentação` ao travamento |
| 08/09/2026 | Passo 4: o Estado 1, e o segundo teste vacuoso | O teste do `Consultar` preenchia sempre `Biblioteca` antes de `Pergunta` e **passava contra uma regra que lia só a segunda**. Virou `it.each` com cada campo vazio na sua vez. Dois vacuosos em quatro passos, os dois por asserção na ordem errada, não por falta de asserção |
| 08/09/2026 | Passo 5: a busca real | O render provisório fica **dentro** do formulário, por um slot `result`, em vez de trocar de tela: sem o `Voltar` do 23-E, uma segunda tela deixaria o painel sem saída. Achado de teste que vale para o próximo corte: `mount()` reinstala o mock, então `mockResolvedValue` definido **antes** dele é descartado em silêncio e o teste passa a medir o default da fábrica — foi o que fez o teste do `no-libraries` se provar não vacuoso por acidente |
| 08/09/2026 | Passo 6: verificação ao vivo pelo dono, e fechamento | Os sete pontos do roteiro conferidos na tela, incluindo uma chamada real: a busca por `tanstack query` devolveu **as cinco bibliotecas com a duplicata de `/tanstack/query`** que a sonda do 23-A previu — a `key` sintética fazendo o que o `id` não faz. O item 7 do roteiro estava mal escrito: pedia comparar a silhueta contra `Paperclip` e `NotebookPen`, que vivem no cabeçalho da conversa como contadores, e o terceiro contador é do **23-H** — a comparação lado a lado só é possível lá, e a formal continua sendo do 23-J. `check:fast` final: **153 arquivos / 1460 testes** |

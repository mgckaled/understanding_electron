...
| `features/open-dataset/OpenDatasetPanel` | 38 | 56 | **mínima** | sai no [plano 16](../active/16-anexo-mecanismo-e-dataset.md), substituído pelo clipe |
...
| 13/08/2026 | 1 | **Passo 5 — folhas** (`Settings`+`LoadedModels` dividem um módulo, `OpenDatasetPanel`, `Versions`; 124 linhas). `Settings` completa. `OpenDatasetPanel`: os seletores descendentes (`.summary div/dt/dd`) viram utilidades nos próprios elementos — `contents` em cada par, `[word-break:break-word]` (propriedade arbitrária) porque nenhuma utilidade equivale exatamente. Gaps de eixo (`gap-x`/`gap-y`) na ordem certa. Comentários load-bearing preservados (D13.6, nome copiável, o `wrap` do rodapé). `check:fast` verde (`loadedModels`, `OpenDatasetPanel`, `Versions` testes intactos), CSS 43,68 → 41,58 kB | Passo 6 — fechamento |
| 13/08/2026 | 1 | **Passo 6 — fechamento. Plano concluído.** `find` confirma **dois** módulos restantes (`MarkdownMessage`, `Dialog`), cada um com no topo por que sobreviveu (o do `MarkdownMessage` ganhou a razão do DS-2). Bundle medido: CSS **50,11 → 41,58 kB** (queda de ~8,5 kB, o saldo negativo que o DS-1 previu); JS estável; 333 testes intactos. **Revisão visual ao vivo nos dois temas** (script Playwright descartável): casca, lista com truncamento e linha ativa, header, `ModelSelector` com badges, composer com medidor, e o modal **centralizado** — a correção de preflight da DS-1 se mantém. Fechado o único aceite que as capturas em repouso deixavam aberto (apontado pelo advisor): o hover na linha revela as ações (`visibility` `hidden`→`visible`) **sem mover o título** — largura e `x` idênticos (`dx`=`dw`=0). Entrada no `HISTORY.md`; armadilha do `min-w-0` escalada no passo 1. Verificação por fidelidade de emissão de CSS + `check:fast` + olho nos dois temas, **não por pixel-diff** (o harness do DS-1 não foi persistido) | **DS-2 concluído.** Próximo: DS-3 |

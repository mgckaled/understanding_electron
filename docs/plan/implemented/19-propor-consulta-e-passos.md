...
| 2026-08-26 | — | **revisão pós-fechamento** | Duas rodadas na mesma sessão. (a) Revisão de fechamento com `advisor`, achando quatro furos que os 821 testes não cobriam — dois bloqueantes (compilador aceitando `value` com `isNull`/`isNotNull`; `attachmentPartOf` mal tipado para `stepProposal`) e dois não bloqueantes (`numCtx` caindo no default do Ollama; `topValues` computado à toa) — os quatro corrigidos com teste novo, `HISTORY.md` corrigido na atribuição do achado do `gemma3:4b`. (b) Clique manual do usuário na GUI real (`pnpm dev`), que `check:fast` nunca alcançaria: diálogo de confirmação com escopo errado (redesenhado duas vezes), card sem saída persistida da conversa (canal `conversation:removeMessage` novo) e um bug real de perda de schema em resultado com zero linhas (`getColumnsObject()` da API do DuckDB) — os três em D19.8 acima, com a armadilha do schema escalonada para `HISTORY.md`. Em aberto, por decisão explícita do usuário: extensão do vocabulário de filtro (`notContains`/`isIn`) para pedidos de exclusão múltipla — gatilho em `ROADMAP § 2`. Seis commits: `bee4f20`, `700afe9`, `aaf311a`, `487c82f`, `b4b9b57`, `93b7d1f`. `pnpm check:fast` verde (832 testes) antes de cada commit. |

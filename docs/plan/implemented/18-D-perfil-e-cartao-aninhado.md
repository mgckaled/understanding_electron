...
|---|---|---|---|
| 20/08/2026 | todos | **concluído** — plano fecha e move para `implemented/` | Terceira ocorrência do *disclosure* (D18B.5), então a régua dos três disparou e a extração aconteceu aqui, como o 18-B já previa. Perfil de nível 2 sobre `SUMMARIZE`, com o cartão aninhado. |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| O `avg` de DATE/TIMESTAMP do `SUMMARIZE` é uma string de data — `Number(...)` vira `NaN` sem aviso | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Top-N só para coluna de baixa cardinalidade; o limiar mora em `core/` | [`ESCOPO.md`](../../ESCOPO.md) |
| Decisões D18D.1 em diante | [`DECISOES.md`](../../DECISOES.md) |

⚠️ **Medição desta sessão não confirmada:** o `check:fast` marcou ~88s, quase o dobro do 18-C para um crescimento de 4 arquivos — medido numa máquina suja (CSV de ~500 MB gerado, processos órfãos do Electron). Remedido a frio no 18-E: **~57s**, confirmando ruído de sessão, não patamar novo. A série completa está no [`ROADMAP § 2`](../../ROADMAP.md).
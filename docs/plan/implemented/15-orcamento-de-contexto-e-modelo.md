...
|---|---|---|---|
| 11/08/2026 | 4 (aceite ao vivo) | **fechado** | O item de aceite que pedia *"comparar o número mostrado com o `prompt_eval_count` e anotar o erro no diário"*, feito depois da D15.14. Mesma pergunta-e-resposta única, dois modelos: **`gemma3:4b` → ~204 tokens** e **`qwen2.5-coder:3b` → ~311**. Os dois plausíveis para os textos em questão, e a divergência entre eles **é o resultado, não ruído** — são tokenizadores de famílias diferentes (SentencePiece contra BPE) e só um injeta system prompt de fábrica. A superestimativa residual do Qwen vem do custo fixo por requisição diluído num turno curto, e cai a partir do terceiro. **Assunto encerrado a pedido do dono**, com gatilho declarado: número muito divergente reabre. |

...
|---|---|---|---|
| 11/08/2026 | 4 (aceite ao vivo) | **fechado** | Estimativa contra `prompt_eval_count` real, mesma pergunta em dois modelos: `gemma3:4b` → ~204 tokens, `qwen2.5-coder:3b` → ~311. **A divergência é o resultado, não ruído** — tokenizadores de famílias diferentes (SentencePiece vs BPE), e só um injeta system prompt de fábrica. Encerrado a pedido do dono, com gatilho: divergência muito maior reabre. |
| 11/08/2026 | 4 (correção) | **D15.14** | O medidor estava pinado no `prompt_eval_count` do turno anterior: `charsPerToken` e `estimated` **se cancelavam**, e a resposta do modelo nunca entrava na conta. **O perigo real era o portão, não o medidor** — com a razão inflada a 31 caracteres/token, um rascunho de 4.000 caracteres somava ~127 tokens em vez de ~1.050, e o portão deixaria passar exatamente a mensagem que existe para recusar. O teste que faltava estava escrito no aceite do passo 4; só metade virou asserção. |
| 11/08/2026 | 6 | **concluído** | A trava do par (modelo, `num_ctx`) implementada. Três coisas só apareceram ao implementar: a trava **invalida a dica do portão** (D15.5 oferecia as duas saídas que a trava fecha); `messages` é vazio enquanto a leitura está em voo, indistinguível de conversa vazia — daí `messagesLoaded`, assumindo travado até saber o contrário, porque destravar por engano é a direção que machuca; e o recuo silencioso do `resolveModel` não podia esperar, ou a trava seria mentira onde mais importa. |
| 11/08/2026 | revisão de decisão | **D15.7 revertida, D15.13 escrita** | Sessão sem código. O par trava no primeiro envio. O argumento decisivo não foi economia de trabalho: é que a flexibilidade produzia **estado sem saída** — trocar para um modelo menor faz o portão recusar corretamente um histórico que já não cabe, e a conversa fica sem continuação nem conserto. Premissa corrigida no caminho: ChatGPT/Claude/Gemini **deixam** trocar no meio — o princípio é de ferramenta local, não universal. |
| 11/08/2026 | correções de uso | **D15.10, D15.11** | Três defeitos vindos do app rodando, **nenhum achado por teste**. O filtro de variantes foi entregue quebrado na primeira tentativa: certo no `ConversationView`, mas o `select` é do `ModelSelector`, que recebe estado bruto — nenhum teste montava a tela com uma variante no catálogo. Uma exceção minha ("variante com system prompt próprio é outro assistente") foi construída e derrubada contra o Ollama real na mesma sessão. |
| 10/08/2026 | 4 e 5 | **concluídos** | Medidor e portão num commit só — dividi-los commitaria um cálculo sem consumidor. Erro de projeto pego por teste de nível 2: `RAM_MARGIN_BYTES` de 3 GiB **contava a ocupação duas vezes**, e o app recusava o próprio modelo padrão (teto 0). A distinção que o argumento exigia ("ocioso agora, vai encher" contra "já cheio") não existe numa leitura só. |
| 10/08/2026 | 3 | **concluído** | `app:memory` nasceu como canal a mais do que o plano previa: não existe **um** número de RAM livre (9 / 7,5 / 6 GB), então lê-se a cada chamada em vez de chumbar. O campo de contexto é não controlado com `key` por conversa, remontando em vez de copiar — contra a armadilha do controle que copia valor assíncrono. |
| 10/08/2026 | 2 | **concluído** | Dois defeitos que só nível 2 pegou: `Field` injeta `id` **clonando o filho**, então envolver um `StateView` fazia o `label for` apontar para nada; e `conversation?.settings.model ?? pending` **vazava entre conversas** — criar a segunda herdava o modelo da primeira. Conserto: ramificar por "existe conversa?", nunca encadear `??`. |
| 10/08/2026 | 1 | **concluído** | A implementação corrigiu o plano: procurar chave pelo sufixo `.context_length` está errado — modelo com visão carrega namespace paralelo (`gemma3.vision.block_count`), e só não quebrava porque o Ollama devolvia as chaves ordenadas. |
| 10/08/2026 | 0 | **concluído** | O fator saiu **1,055** — a aritmética estava certa. Duas surpresas do outro lado: metade da frota **não cabe** a 32k no cenário de 6 GB (o teto de RAM age no caso comum, não num extremo), e o `gemma3:4b` **cabe** nos 131.072 que declara — o que o mantém fora é o prefill de ~87 min, não a memória. |
| 10/08/2026 | revisão do plano | plano reescrito | Frota 10 → 14. **A manchete do plano estava generalizada demais:** os 120 MB medidos em 09/08 eram do `gemma3:4b`, o único com janela deslizante. Daí nasceram o teto de RAM (D15.2) e o passo 0. `mistral:7b` e `llama3.1:8b` desinstalados ao fim (D15.8). |
| 09/08/2026 | — | plano escrito | Sessão de medição, sem código. As sondas derrubaram duas premissas escritas: `/api/tags` **não** reporta `vision`, e `num_ctx` **não** é um botão de RAM (8× de contexto custa 120 MB). |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| `os.freemem()` no Windows é o *Disponível*, não o *Livre* — o *Livre* daria teto zero a tudo | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| `user.type` despacha tecla por caractere; 4.000 estouram o timeout — use `user.paste` | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Margem absoluta subtraída antes de uma divisão não é custo fixo | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Flexibilidade que produz estado sem saída é pior que a ausência dela | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Não existe razão caracteres-por-token: é do tokenizador, e há custo fixo por requisição | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Um teste que passa com o defeito presente não estava provando nada | skill [`testing`](../../../.claude/skills/testing/SKILL.md) |
| Peso e cache KV por faixa de contexto, por modelo | [`reference/models/`](../../reference/models/README.md) |
| Decisões D15.1–D15.14 | [`DECISOES.md`](../../DECISOES.md) |

⚠️ **Régua pendente registrada na época, não resolvida aqui:** `useConversationChat.ts` ficou em 200 linhas contra o teto de 120 de hook — já estava em 188 antes da sessão, e dividir junto da trava seria uma segunda variável.

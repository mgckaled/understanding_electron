...
Conserto: `resultError(value): string | null` (`core/observatory/events.ts`) reconhece a forma `{ ok: false, error: { kind } }` e devolve o `kind` como string. `wrap()` agora computa `error = resultError(result)` uma vez, após `fn(args)` resolver, e passa o mesmo valor tanto para `record()` (contador `AppIpcStat`, painel Canais IPC do O-2) quanto para `emitEvent()` (sink do O-6). Nenhum teste automatizado (`check:fast`) pegaria isso sozinho — só apareceu matando o Ollama de verdade e olhando o painel vazio onde uma linha era esperada.

### DO6.11 — Canais IPC (O-2) também passa a contar `Result.ok:false` — pedido do usuário, mesma sessão

Ao ver o item 4 do roteiro corrigido no painel Eventos, o usuário notou que o painel **Canais IPC** (`Em andamento`, do O-2) continuava mostrando `ai:chat` com `0` falhas para a mesma chamada. A tentação de tratar isso como "fora do escopo do O-6, mexe em O-2" foi descartada depois de conferir a fundamentação do próprio O-2 (`plan/implemented/O-2-...md`): **DO2.4** só fixa que `lastError` é pegajoso (não que se reseta), e **DO2.8** só exclui payload zod inválido do contador — nenhuma das duas jamais decidiu que `Result.ok:false` não deveria contar. Era uma lacuna não testada (`ai:chat` nunca tinha sido derrubado de verdade num teste do O-2), não uma decisão a reabrir. Como o `resultError(result)` já era calculado no mesmo ponto para o sink do O-6, alimentar `record()` com o mesmo valor custou uma linha — os dois painéis (Canais IPC e Eventos) agora concordam sobre o que conta como falha para o mesmo `ai:chat`.
...
|---|---|---|
| 01/09/2026 | 1 | Escopo do O-6 fechado em conversa (fonte do evento = hook único em `ipcStats.wrap`, retenção configurável com intervalo fechado 7–90 dias, textos de transparência no cabeçalho do painel e em Configurações). Plano escrito e revisado pelo advisor (Opus) antes de codar — corrigiu o desenho do sink (setter mutável, não parâmetro de construtor) e o schema (sem `STRICT`/`AUTOINCREMENT`). Context7 confirmou a API do `node:sqlite`. **Todos os 8 passos implementados na mesma sessão**, um commit por passo (`63386c5`..`9371346`): schema, extração pura + sink, gravação real, retenção, wiring no boot, contrato IPC, campo em Configurações, painel Eventos. Segunda revisão do advisor, agora sobre o código, achou dois problemas visíveis em uso — auto-instrumentação do canal `events:list` e janela de retenção que só ficava verdadeira após um restart — corrigidos como DO6.9 (ver Decisões), mais `EventRow.id` para chave estável de linha no React. **Verificação ao vivo feita pelo usuário**, 7 itens do roteiro: 1, 2, 3, 5, 6 e 7 passaram de primeira; o item 4 (forçar falha do Ollama) revelou um problema real — nenhuma linha de erro aparecia, nem no painel Eventos nem no contador em memória de O-2. Causa raiz e conserto: DO6.10 (`resultError`, `Result.ok:false` reconhecido em `wrap()`). **Reteste do item 4 confirmado pelo usuário** (print: `ai:chat`, `error: unavailable`, `domainId` presente) — os 7 itens do roteiro fecham nesta sessão. Na sequência, o usuário notou que o painel Canais IPC (O-2) ainda mostrava `0` falhas para o mesmo `ai:chat` e pediu para incluir o conserto no escopo do O-6; conferido contra `plan/implemented/O-2-...md` que DO2.4/DO2.8 nunca decidiram excluir `Result.ok:false` — era lacuna não testada, não decisão a reabrir — e unificado em DO6.11: `record()` e `emitEvent()` agora recebem o mesmo `error`. `pnpm check:fast` final: 139 arquivos de teste, **1237 testes**, zero erros de lint. Skills usadas: `architecture`, `ipc`, `data`, `testing`, `design-system`, `comments`. |

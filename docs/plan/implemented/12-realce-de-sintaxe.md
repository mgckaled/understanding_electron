...
|---|---|---|---|
| ago/2026 | todos | **concluído** | `highlight.js` escolhido e `shiki` descartado por acoplamento à CSP — a mesma régua que a fase 03 fixou decidiu aqui. `detect: false` (D12.5): sem linguagem declarada, não adivinha. |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| `highlight.js` sobre `shiki`, e o porquê da CSP | [`HISTORY.md`](../../HISTORY.md) |
| O `github.css` desatualizado reprovava AA (D12.3); a opção `languages` do `rehype-highlight` não encolhe o bundle | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| `--syntax-*` não corresponde nome a nome ao vocabulário do `highlight.js` | skill [`design-system`](../../../.claude/skills/design-system/reference.md) |
| Decisões D12.1–D12.5 | [`DECISOES.md`](../../DECISOES.md) |

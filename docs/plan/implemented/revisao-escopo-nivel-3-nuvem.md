...
3. [ ] Verificação ao vivo (anexar CSV real com Gemini/GLM configurado) — **não executada nesta sessão**: exigiria launch do app + chave de API real, e a regra de nunca ler `.env` já impede confirmar se uma está configurada sem perguntar. Cobertura automatizada é forte (item 4), mas não é a mesma prova. Fica como verificação pendente para quem tiver a chave à mão

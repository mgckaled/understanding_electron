...
✅ **Aceite observado pelo usuário em 27/08/2026.** As dez conferências do passo 5 certas, e **duas rodadas de conserto entre elas**: o `.txt` que saía escapado (DE1E.9) e o `.docx` que saía sem tabela e sem espaçamento (DE1E.10, DE1E.11). Fechado com *"esse ficou perfeito agora"* sobre uma resposta de 63 parágrafos com tabela de 20 linhas, e sobre uma segunda com bloco de código. **Plano concluído** — resta o **E-1-F** (`.pdf`), único da trilha com bifurcação em aberto.

| Data | Passo(s) | Estado | Observação |
|---|---|---|---|
| 27/08/2026 | 5 | **prova ao vivo — dois defeitos, ambos meus** | **O `.txt` saía escapado**, com `&#x20;` no lugar da indentação: eu tinha consertado o sumiço de código na árvore e não olhado o serializador logo abaixo (DE1E.9). **O `.docx` saía sem tabela e sem espaçamento** — a primeira exportação real já disparou o gatilho que eu havia adiado, e o `styles.xml` que o `docx` gera é vazio de folga, o que era um defeito maior que a tabela (DE1E.10, DE1E.11). ⚠️ **O que dói é que os dois eram previsíveis pelo que eu mesmo já sabia:** eu havia descrito a classe de dano do escape ao analisar o pilar Código, **na mesma sessão**, e escrito o gatilho da tabela com as palavras exatas que a prova ao vivo cumpriu. Saber e não aplicar é uma falha diferente de não saber. **O que salvou o custo foi a arquitetura da DE1E.9** — quando a tabela chegou, absorver custou um tipo e um ramo em cada renderizador, em vez de retrabalho nos dois caminhos. |

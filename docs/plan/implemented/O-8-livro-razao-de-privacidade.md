...
| 01/09/2026 | Implementação completa: schema, `core/observatory/privacy.ts`, `main/observatory/privacy.ts`, retenção, contrato IPC, `chat()` e `propose()` instrumentados, painel Privacidade, sincronização de docs | 12 commits, um por passo. `pnpm check:fast` verde (146 arquivos, 1283 testes). Teste de ausência (`ollama` não grava) verificado vermelho antes de verde, manualmente, nos dois handlers |
| 01/09/2026 | Verificação ao vivo — chamada real ao Gemini com documento anexado, painel confirmado | Achado ao vivo: cabeçalho mostrava "1 chamadas" no singular real — corrigido na hora. Plano movido para `implemented/`, marco fechado em `HISTORY.md` |

...
|---|---|---|---|
| 18/08/2026 | 1 | **Plano concluído, 15 passos, um commit cada.** `HISTORY.md` caiu de ~184 mil para 77.681 caracteres (58% menor), com 24.634 migrados para o `HISTORY-archive.md`, que nasce aqui. Medido com `wc -m`, não `wc -c`: **byte não é caractere, e a diferença importa em português acentuado**. | concluído |
| 18/08/2026 | 1 | **O checklist de auto-conservação do passo 1 pegou algo três vezes** — e o terceiro é o que justifica a régua: recontar a suíte no passo 14 achou um drift que a auditoria não tinha quantificado (de ~15s para ~50s). Os dois primeiros (grep de nome antigo, recontagem de canais) confirmaram valores já citados — mas **verificados, não copiados**. | — |
| 18/08/2026 | 1 (advisor, pós-fechamento) | **Seis marcos tinham perdido a alternativa descartada durante a compressão** — achado comparando `git diff` entrada por entrada, não relendo. O item de verificação existia e tinha sido **citado como feito**. Todos restaurados num commit separado. | corrigido |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| Comprimir apaga a alternativa descartada antes de qualquer outra coisa — o `git diff` é a verificação | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| O mecanismo de arquivamento e a régua de compressão | [`docs/README.md`](../../README.md) |
| Decisões R2.x | [`DECISOES.md`](../../DECISOES.md) |
...
| 3 (03/09/2026) | Usuário leu a skill publicada e apontou narrativa/proveniência que sobrou (títulos citando "migrado de CLAUDE.md, R-6", "Ancoramento pós-fato (21-C-A) — por que..."), pedindo checagem contra a mesma régua da skill `comments`: sigla aponta, não conta a história. Terceira consulta ao `advisor`: confirmou os dois achados e mais dois (comparação ao `make_llm_fn` do mill.tools, "duas medidas já usadas" dentro de uma citação de sigla) — todos cortados, mantendo o fato e a sigla nua. Seção inteira "O corte do arco 21" removida — era quadro de status, dono é `ROADMAP § 1`. Pergunta seguinte do usuário — concentrar toda referência temporal/sigla num só lugar, em vez de inline — respondida com o `advisor`: **não**, contraria a própria regra que a skill `comments` já fixa (`(D15.2)` no ponto da afirmação, não num apêndice) e tornaria `ai` a única das sete skills a divergir do padrão. `wc -l` final: 150 linhas. |
| 4 (03/09/2026) | Usuário achou erro factual na skill: "`qwen3:4b` é o único com `thinking`" — falso, herdado sem verificar do `CLAUDE.md` original. `qwen3.5:2b` também tem `thinking`, mais visão, e é o mais usado na prática (captura de tela do painel Desempenho do Observatório: mediana ~38 tok/s, N=11, muito acima dos ~5 tok/s dos modelos maiores). Corrigido o one-liner. Investigação em dois passos, cada um com consulta ao `advisor`: (1) medição ao vivo (`/api/show`, `/api/tags`, só metadado) achou dois problemas reais, não só a linha errada — `capabilities: ['completion','vision','tools','thinking']` juntas (primeira vez na frota, dispara o gatilho do `ROADMAP § 2` sobre vision+tools) e `attention.head_count_kv: null` (mesmo defeito silencioso já documentado para `granite4`, mas este modelo **está** na frota e é o mais usado — o orçamento de contexto não custeia `qwen3.5:2b` hoje). (2) usuário pediu classificação de trilha para o achado maior; `advisor` separou os dois por teste distinto: o bug de `head_count_kv` é entrada de `ARMADILHAS.md` + fix que anda com o próximo toque em `core/ai/models.ts` (não merece plano próprio — "divide-se ao tocar" aplicado a planos); o gate vision+tools é mudança de comportamento visível, logo trilha `F` (R3.1), não `09` (fatia restante é só RAG/ML) nem arco `21` (raciocínio, 21-D é o próximo corte) — nasce **F-6**, registrado no `ROADMAP § 1` como planejado sem arquivo, e o gatilho do `ROADMAP § 2` marcado disparado. `docs/reference/models/README.md` ganhou a linha do `qwen3.5:2b` (medida, não estimada) e perdeu as duas claims "único com X" que a nova medição invalidou; aviso deixado no próprio documento de que o reinventário completo da frota (a tabela é de 18/08/2026) segue pendente, fora de escopo aqui. |

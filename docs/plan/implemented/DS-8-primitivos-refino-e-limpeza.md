...
- **DS8.6 — `advisor` chamado três vezes, cada uma mudando o plano.** Antes do parecer (redirecionou a leitura de S1/S2, pediu os três greps que viraram os achados de `Toolbar` zero-consumidor, teste-count e `onChangeCommitted`); antes de codificar (recusou `opacity-0` para o `Button`, recusou a troca de `role="alert"` no `Field`); ao final (confirmou o plano sem achado novo, diferente do DS-6/DS-7/R-4 — não há garantia de que a terceira chamada sempre encontre algo, e não encontrar não é sinal de chamada desperdiçada).

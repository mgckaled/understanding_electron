...
| 15/08/2026 | 0–6 | **plano concluído** — move para `implemented/` | Sessão única, os sete passos, sete commits. Antes de tocar código: confirmado com o usuário que o clipe fica **dentro** do composer — a dúvida era memória de uma decisão nunca escrita; o código (DS-5, commit `0ac70e0`) e o próprio texto do plano (D16.6) já apontavam para dentro. **Passo 0** mediu 51/104/180 tokens (5/20/40 colunas, `gemma3:4b`, `num_ctx=8192`) — "centenas", nunca "milhares"; D15.3 fechado como não-requisito. **Passo 1**: `MessagePart` vira união discriminada, `core/ai/dataCard.ts`, `toChatMessages` materializa a variante `dataset` e `messageText` permanece só-texto de propósito. **Passo 2**: duas armadilhas reais, subidas ao `HISTORY.md` na mesma sessão — `crypto.Hash` **não é passthrough** (achado no Context7 antes de escrever código errado: a interface de stream só emite o dígest final, uma vez) e `StringDecoder` é obrigatório contra corrupção de acento na fronteira de chunk (provado fatiando "endereço" no meio do "ç" e comparando hashes). **Passo 3**: `features/open-dataset/` vira `features/attachment/`, `AttachButton` fica controlado, `dataset:scan`/`useOpenDataset` saem por não terem mais chamador. **Passo 4**: `DatasetCard` desenha o anexo na conversa; `MarkdownMessage` sobe para `shared/ui/`, fechando o gatilho do `ROADMAP § 2`. **Passo 5**: as duas fugas do medidor (D16.5) — histórico e anexo pendente — provadas **vermelhas** com dois testes escritos antes do conserto, corrigidas uma de cada vez. **Passo 6**: desenho mais simples que o rascunho original — recalcula o conjunto global de hashes **depois** do `DELETE` em vez de capturar "antes"; roda no `conversation:remove` e uma vez no boot, fechando sem canal novo a lacuna que a D16.2 já previa em prosa. Verificado em quatro níveis: `check:fast` (375 testes), e2e dev rodado duas vezes ao vivo (build real, 5/5 nas duas rodadas), os dois testes de nível 2 vermelho-antes, e revisão dos documentos vivos que apontavam para este plano (`ROADMAP.md`, `plan/active/README.md`) antes de mover. Entrada em [`HISTORY.md`](../../HISTORY.md). |

...
| 08/09/2026 | Passo 2: o trecho abre | Vermelhos provados numa sabotagem tripla. ⚠️ **A primeira tentativa não rodou teste nenhum**: o `\&` escapado pelo shell entrou no `.tsx` e o arquivo virou erro de sintaxe — `Tests no tests` é o sintoma, e ele **não** é um vermelho, é a suíte não tendo corrido. Virou entrada própria no [`ARMADILHAS.md`](../../ARMADILHAS.md). O quarto teste do passo confirma o que a skill `testing` manda conferir antes de mandar algo para nível 4: o destaque de sintaxe **cabe** em jsdom pela classe (`.tok-keyword`), só a cor é que não |
| 08/09/2026 | Verificação ao vivo pelo dono: os oito pontos, e um defeito que só o olho pega | Consulta real a `/tanstack/query`: **5 trechos, 0 notas, 741 tok**, e a soma conferida no print (85+108+190+284+74). ⚠️ **A descrição do trecho é markdown, e saía crua** — `` `invalidateQueries` `` com os acentos graves à vista. Descrição e nota passaram para o `MarkdownMessage`; o argumento que decidiu não é o acento grave, é que as duas são texto de terceiro (D11.2), e um partidor de acento grave escrito aqui abriria um segundo dono de markdown. As regras ficam em texto puro **de propósito**: são o registro do que o serviço tentou injetar. Dois pedidos do dono atendidos: o código subiu de `text-sm` para `text-md` (15px → 16px, um passo da escala e não um valor arbitrário) e a linha do trecho ganhou hover pela escada `bg-surface` → `bg-surface-raised`. Uma segunda consulta (`/pandas-dev/pandas`) confirmou a aba Notas com conteúdo real, trilha e chip de código inline. **Ressalva registrada e não consertada:** com a descrição em leitura (18px) e o título do trecho em chrome (15px), a legenda é maior que o título que ela legenda — decisão do dono ao pedir mais tamanho; se incomodar, o conserto é subir o título |
| 08/09/2026 | Fechamento | `check:fast` remedido: **154 arquivos / 1488 testes, 106,34 s**. Errata devolvida aos três documentos do guia — três premissas no `README.md`, o aviso do topo do `painel.md` reescrito com o Estado 3, e **três** decisões no `decisoes.md` (DM-17 pela procedência das listas, DM-28 pela descrição que a tabela dela não previa, DM-16/25 pelo adiamento que continua). A 11ª entrada de marco empurrou o 21-D-A para o `HISTORY-archive.md` na mesma edição. Contagem envelhecida achada de graça: o cabeçalho do `ARMADILHAS.md` dizia **106** entradas quando havia 110 — remedido para 111 com a entrada nova. `CLAUDE.md` remedido em nove números (a pasta `docs/` foi de ~2,51 MB/693k para **~2,57 MB/709k** em 128 arquivos) |
| 08/09/2026 | Duas skills atualizadas | Conferidas as sete. Sem deriva: `ipc` (53 canais, nenhum criado — recontado no `IpcContract`), `architecture` (oito fatias, nenhuma nova), `data`, `ai`, `comments`. Mudaram duas. **`testing`** ganhou duas regras que este corte pagou: vermelho tem **uma** forma (`Tests N failed` com o nome do teste), e `Tests no tests` é a suíte não tendo corrido — mais o achado de que **sabotagem larga demais prova menos**, porque anular a tela inteira deixou sem exercício a segunda asserção do mesmo teste. **`design-system`** ganhou a régua do `MarkdownMessage`: ele não é "o corpo da mensagem do modelo", é o dono do markdown, e o critério de usá-lo é **procedência** — texto de fora chega com marcação e é entrada não confiável. Com o aviso de que ele fixa 18px na própria folha, que é a origem da ressalva de hierarquia da linha acima |

...
|---|---|---|---|
| 13/08/2026 | 2 | **Desvio medido, com a alternativa recusada por razão e não por gosto:** `useAiAvailability` ficou em `useState`+`useEffect`, **não** `useQuery` — a cadência da query resolve o *availability* um tick depois do medidor, e o gate do composer corria com o teste que digita, deixando dois testes de orçamento falhando de forma **não-determinística**. Desvios também documentados como recusa consciente: sem alternador de tema, sem a grade 2/4/6 de threads (a máquina tem 8). | concluído |
| ago/2026 | — | **A interface chega ao alvo e a trilha DS encerra** (antes de DS-4..DS-8, que vieram de relatórios externos posteriores). O desmonte da toolbar aconteceu aqui — foi o que deixou o primitivo `Toolbar` sem chamador, apagado só no DS-8, duas migrações depois. | concluído |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| A barra de acento de seleção: borda + fundo + peso, largura sempre reservada | skill [`design-system`](../../../.claude/skills/design-system/SKILL.md) |
| Decisões DS3.x | [`DECISOES.md`](../../DECISOES.md) |
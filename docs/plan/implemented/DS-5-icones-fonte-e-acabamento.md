...
|---|---|---|---|
| 15/08/2026 | Sessão 1 | Fase 0: `ds-4` mergeado em `main` (fast-forward) e empurrado; `docs/DS-04/` removido (60 arquivos); link morto convertido; plano registrado. Fase 1: `lucide-react` instalado, `shared/ui/icon.ts` criado, os nove glyphs Unicode migrados nos 6 arquivos, mais `Pencil`/`Trash2` no menu kebab — verificado ao vivo (engrenagem visivelmente maior). Fase 2: `@fontsource/jetbrains-mono` instalado, `--font-mono` atualizado, `BRIEF-claude-design.md` corrigido — **achado ao vivo, fora do plano**: o import `400.css` trazia subsets que o CSP bloqueava como `data:` URI (console com 4 erros); corrigido para `latin-400.css`, escalado para `HISTORY.md` como armadilha generalizável. Fase 3: hipótese nula confirmada duas vezes (teste de componente jsdom com/sem rótulo de cerca; e ao vivo contra o Ollama real, `qwen2.5-coder:3b`, aguardando o fim do streaming) — o mecanismo de realce nunca esteve quebrado, `detect: false` (D12.5) está se comportando como decidido. Fase fechada sem alterar `MarkdownMessage.tsx`. Fase 4: escala do chrome (2xs/xs/sm/md sobem 1px, lg parado), auditada ao vivo com sidebar semeada via IPC direto (`conversation:create`/`append`, sem depender do Ollama) contra `notes/1.png`/`5.png`; fit dos `Button` verificado antes de commitar, não suposto. Fase 5: `ModelSelector.tsx` dividido em `ModelPicker`/`ContextControl` (DS5.6), `AttachButton.tsx` novo substituindo `OpenDatasetPanel` (DS5.5) — ordem clipe·modelo·contexto·recarregar confirmada ao vivo; `modelSelection.test.tsx`/`contextBudget.test.tsx` passaram **sem alteração** (consultam por role/nome acessível, não por agrupamento de popover). Fase 6: botão de copiar no bloco (`useCopyToClipboard`, `textOf`/`trimTrailingNewline`), verificado com clipboard real. Fase 7: `TurnActions.tsx` — copiar funcional, compartilhar/atualizar `disabled` de verdade (DS5.7). Fase 8: QA final nos dois temas com sidebar+conversa+configurações+kebab semeados via IPC (`emulateMedia`); e2e dev rodado — **achado**: `open-dataset.spec.ts` ainda clicava `Escolher arquivo` direto, sem abrir o clipe primeiro; corrigido (abre `Anexar arquivo`, clica, reabre para ler o resultado). 5/5 e2e verde. `check:fast` verde (348 testes) em toda fase. Bundle final: CSS 44,96 → 47,00 kB, JS 1.571,29 → 1.584,44 kB. **Revisão do usuário, mesma sessão, apontou cinco desvios do padrão vivos após o passo 8** (fora do que qualquer fase tinha verificado ao vivo, porque nenhuma comparou o *box model* dos botões de ícone-só): `Button` ganhou o eixo `shape="square"` (nenhum ícone-só usava padding de botão de texto — engrenagem, clipe, colapsar, fechar do `Dialog`, kebab e recarregar corrigidos, o último migrado de `<button>` cru para `Button`); `+` virou `Plus` com `gap-2` num `<span>` interno (`Button` embrulha todos os filhos num único `<span>`, então o `gap-3` da base nunca separava `+` do texto); kebab mostra só "Editar título"/"Excluir", com o título completo migrado para `aria-label` (preserva os testes existentes); `Field` ganhou a prop `inline` (rótulo ao lado do controle), usada nas duas pílulas — resolve o clipe "flutuando" ao lado de um bloco de duas linhas; `ICON_SIZE` sobe 2px em cada degrau e o texto das opções do popover de modelo vai de `text-xs` para `text-md`; `Sidebar` troca `ChevronsLeft/Right` por `PanelLeftClose/Open`. Um commit de correção, verificado ao vivo nos dois temas (box-model antes/depois), `check:fast` e e2e 5/5 de novo verdes. | **DS-5 concluído**, com a correção de padrão da mesma sessão. Nove fases, um commit cada, mais dois commits de correção (CSP na Fase 2, ícones/alinhamento após o passo 8) |

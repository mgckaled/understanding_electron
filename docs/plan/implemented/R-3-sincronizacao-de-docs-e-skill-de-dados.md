...
|---|---|---|---|
| 22/08/2026 | 2 | **Plano concluído, 10 passos, um commit por passo.** Nasce a skill `data`. Achado de bônus no passo 2: **Parquet nunca foi suportado pelo seletor de arquivo**, apesar de citado como formato pronto. Context7 consultado antes de cada afirmação técnica; as três regras novas do `guard.mjs` testadas por provocação (editar um arquivo que viola cada uma, confirmar `exit 2`, reverter). | concluído |
| 22/08/2026 | 2 (advisor, ao final) | Três problemas reais, todos da mesma família — **afirmação escrita a partir de prosa de outro documento, não do código**: identificadores da skill nova citados sem grep (um estava errado); o peso da extensão excel copiado do `CLAUDE.md` em vez de remedido (`ls -l` confirmou o número, mas **por sorte, não por verificação**); e a entrada em `HISTORY.md` sem a alternativa descartada — a mesma classe de defeito que o R-2 cometeu seis vezes. | corrigidos num commit próprio |
| 22/08/2026 | 1 | Plano registrado. Nasceu de uma auditoria crítica de `.claude/` e dos dois READMEs, com **cada achado verificado contra o repositório real antes de entrar no plano**. | passo 1 na sessão seguinte |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| Parquet citado como formato pronto e nunca listado no seletor | skill [`data`](../../../.claude/skills/data/SKILL.md) + [`ESCOPO.md`](../../ESCOPO.md) |
| A camada de dados inteira (motor restrito, Arrow, extensão vendorizada) | skill [`data`](../../../.claude/skills/data/SKILL.md) |
| Decisões R3.x | [`DECISOES.md`](../../DECISOES.md) |

⚠️ **Registrado como não verificado:** `test:e2e`/`test:e2e:packaged` foram editados no passo 4 e **nunca rodados** — `check:fast` não os alcança, e a mudança era sintática o bastante para não justificar um build completo. Fica como não medido, não como verificado.
...
| 12/09/2026 | Passo 5: verificação ao vivo pelo dono — **doze de doze pontos**, e a sonda que fechou a última verificação aberta do arco | Os onze textos conferidos na tela, incluindo os três que só se alcançam quebrando algo de propósito (chave removida, chave inválida, rede desligada). ⚠️ **A sonda dos dois regimes respondeu, e a prova decisiva veio da repetição, não da primeira leitura.** A primeira rodada mostrou 55 gastos no anônimo contra 201 na chave no mesmo instante — forte, mas inferência sobre o acumulado. O dono repetiu as duas chamadas e **cada contador caiu exatamente 1** (anônimo 145 → 144, chave 799 → 798): fosse um contador só, a chamada autenticada teria derrubado o anônimo junto, para 143. Quatro chamadas ao todo, mesmo `Ratelimit-Reset` nos dois. **Dois achados de graça:** o contador anônimo tinha 146 em 07/09 e marcou 145 logo depois desta sonda, ou seja, perdeu **exatamente** a chamada dela — prova de que o app vinha consultando com a chave desde o 23-B, o que se supunha e nunca se conferira; e os 201 gastos são o custo real do arco 23 inteiro até aqui. ⚠️ **Separados não quer dizer somáveis** (D23I.13): a queda automática para o anônimo segue recusada, porque reintroduziria o silêncio que motivou exigir a chave. A hipótese do *pool global compartilhado* não foi nem sustentada nem derrubada — duas chamadas não distinguem "por cliente" de "por IP" |
| 12/09/2026 | Fechamento | Plano movido para `implemented/`, entrada de marco no `HISTORY.md` — e a **11ª empurrou o R-7 para o `HISTORY-archive.md` na mesma edição**, como a régua (d) manda. Errata devolvida aos três documentos do guia: `README.md` (o 23-I entregue, **três** premissas caídas e as duas verificações abertas fechadas), `decisoes.md` (DM-12 virando onze e perdendo a ação "abre Configurações"; DM-19 com a metade "opcional" cumprida) e `api.md` (a sonda, a ausência de endpoint de cota, os nomes dos headers). Pendência do `ROADMAP § 3` sobre exigir chave **removida** — deixou de ser pendência. Uma skill mudou (`ipc`: 53 → **54** canais, e a linha do domínio `docs` com o porquê de `quota` não devolver `Result`); as outras seis conferidas sem deriva. `CLAUDE.md` remedido em **seis** números: `docs/` foi de ~2,60 MB/716k para **~2,67 MB/735k** em 131 arquivos, `plan/implemented/` de 82 para 84 arquivos (~427k), e as armadilhas de 113 para **115** — o cabeçalho do `CLAUDE.md` estava dois atrás do próprio `ARMADILHAS.md`, que já dizia 114 |

...
| 31/08/2026 | 1–5 | passos 1–5 entregues; **falta o passo 6 (conferência ao vivo, do usuário)** | `pnpm check:fast` verde, 1185 testes. Sondagem ao vivo contra o motor real (não só a doc) confirmou os nomes de coluna antes de escrever `handleEngineInfo` — as três queries de `core/duckdb/engineInfo.ts` rodaram contra `DuckDBInstance.create(':memory:')` de verdade (`engineInfoLive.test.ts`). Todas as provocações do plano provadas em vermelho ao vivo nesta sessão, não só escritas: o filtro de extensões, `readEngineInfo` engolindo a exceção sem `err(...)`, e a DO3.4 (lista hardcoded reprova, `sqlite_master` passa). O teste do O-1 que provava "só grupo com painel aparece" precisou de ajuste — Armazenamento ganhou painel, como o próprio DO1.10 previu; não é regressão. Achado sob carga, não no design: com 5 módulos de painel no mesmo chunk lazy, o `findByRole` de 1 s do RTL passou a estourar só na corrida completa da suíte (127 arquivos em paralelo) — corrigido com timeout explícito. Uma segunda falha na mesma corrida (`ConversationView.test.tsx`) é flakiness pré-existente sob carga, já presente no início da sessão antes de qualquer mudança do O-3 — confirmada isolada (34/34), fora do escopo deste plano. |
| 31/08/2026 | revisão advisor + passo 6 | plano concluído — todos os 6 passos entregues e conferidos ao vivo | Revisão do advisor Opus achou dois pontos reais: `formatSize()` (GB fixo) fazia memória por tag e tamanho do `crivo.db` — ambos tipicamente MBs — aparecerem como "0,0 GB", indistinguível de vazio; e a skill `ipc` estava desatualizada (contagem e a linha `Result?` do domínio `dataset`, agora mista entre `queueDepth` e `engineInfo`). Corrigido com `formatBytes()` nova (escala automática B/KB/MB/GB) só para os dois casos pequenos — os sete call sites existentes de `formatSize` ficaram intocados porque dependem da escala fixa em GB para comparabilidade entre si (RAM livre/total, peso de modelo), confirmado por grep antes de decidir por duas funções em vez de uma. Cada troca provada vermelha antes de aceitar verde (skill `testing`): nenhum teste de painel afirmava a string renderizada até então, então trocar a formatação sem essa asserção teria ficado verde sem provar nada. **Teste ao vivo do usuário achou um bug real que nenhum teste automatizado alcançava**: o painel Motor DuckDB mostrava "Permission Error: Cannot access directory ...\.duckdb\extensions\..." em vez das extensões — `extension_directory` nunca era definido, ficava no default do SO fora de `allowed_directories`, e `duckdb_extensions()` sonda esse caminho para reportar `installed`, batendo em `enable_external_access = false`. A sondagem ao vivo do passo 1 não pegou porque rodava contra `DuckDBInstance.create(':memory:')` nua, sem a sequência de startup restrita do worker real — lacuna corrigida com um segundo `describe` em `engineInfoLive.test.ts` que roda `buildDuckDbStartupCommands` de verdade e reproduziu o erro exato do usuário antes do conserto (`SET extension_directory` apontando para `tempDirectory`, mesma posição de ordem que `allowed_directories`/`temp_directory` já ocupavam). Conferência ao vivo final: painel Motor DuckDB correto (limite de memória, 6 extensões carregadas com versão real); "Memória por categoria" ausente é comportamento esperado — confirmado ao vivo que `duckdb_memory()` reporta as 16 tags a zero numa instância sem query rodada, e o painel esconde a seção vazia por desenho (DO3.7), não trata ausência como zero. Painel Banco de dados confirmado correto pelo usuário. `pnpm check:fast` fechou 1187/1187. |

...
|---|---|---|---|
| 18/08/2026 | — | **plano concluído**, movido para `implemented/` | Verificação ao vivo completa pelo usuário: os três tipos numa conversa só, `gemma3:4b` lendo os 4 formatos de imagem, cancelamento, persistência e recusa de PDF escaneado. `build:win` verde — o protocolo `attachment://` e o rasterizador sobrevivem ao app empacotado. |
| 18/08/2026 | 8 (fechamento) | checklist fechado; `ImageCard` ganha cabeçalho clicável | Achado ao vivo: `ImageCard` mostrava a imagem cheia, sem nome nem extensão — inconsistente com `DocumentCard`, que nasce colapsado (D17.9). Reescrito no mesmo formato. O selo de extensão lê `part.fileName`, **não** `part.mimeType`: SVG/WebP normalizam para PNG no disco, e rotular "PNG" um arquivo escolhido como `.svg` mentiria. |
| 17/08/2026 | 7 | concluído | Spike ao vivo decidiu a D17.7 no candidato 2 (`executeJavaScript` + `canvas.toDataURL`), sem precisar testar 1 e 3. `attachImage` virou dispatcher testável em nível 1; `rasterizeToPng` é main-only, sem teste abaixo do nível 4. 13 testes novos. |
| 17/08/2026 | 6 | concluído | Gate de visão (D17.11) nos **dois** pontos independentes — compose (`AttachButton`) e send (`Composer.canSend`) —, cada um chamando `hasCapability` com o próprio `model`, nunca um booleano computado uma vez. `budgetFor` ganhou `flatTokens`; `setLastPrompt` pula a calibração de `charsPerToken` quando há imagem no histórico (D17.12), para o custo fixo não envenenar os turnos seguintes. 12 testes novos. |
| 16/08/2026 | 5 | concluído (mecanismo) | Canais `image:*` + protocolo `attachment://` (`registerSchemesAsPrivileged` em module scope; `main/index.ts` fechou em **exatamente 100 linhas**, o teto). `contentOf` extraída para `toChatMessages` e `toChatMessagesWithImages` nunca poderem divergir. Ajuste de sequência: `ImageCard` teve de vir aqui, não no passo 6 — o `switch` exaustivo quebrou o typecheck assim que a união ganhou o terceiro membro. |
| 16/08/2026 | 4 | concluído | `ai:chat` passou a carregar `Message[]`; renderer e main usam **a mesma** `toChatMessages`, sem materializador paralelo. Medido, fechando o risco 4: zod sobre a união custa **0,02–0,07 ms/parse**, e escala com contagem de partes, não com tamanho de texto. |
| 16/08/2026 | 3 | concluído | `unpdf` instalado (zero sub-dependências). Recusa `blocked` para PDF sem camada de texto (D17.3, sem OCR). Corrigiu um bug anterior ao plano: `errorMessage()` mostrava "Operação bloqueada." genérico em vez do `.reason` (D17.8) — invisível até então porque o único uso de `blocked` nunca chegava à tela. |
| 16/08/2026 | 2 | concluído | Canais `document:*`, extrator `.txt`/`.md` com fallback windows-1252 (D17.13). `useAttachDataset` virou `useAttachFile<T>` com a API escolhida **na chamada**, não guardada em estado — evita a race de closure obsoleta que `useState` criaria. `mapFsError` extraído antecipando a régua dos três. |
| 16/08/2026 | 0–1 | plano escrito; passos 0–1 | Desenhado com 3 agentes Explore + 1 Plan, revisado pelo advisor em duas rodadas. A primeira reconciliação do gate de visão (D17.11) uniu as duas respostas em vez de escolher — corrigida antes do `ExitPlanMode`. |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| Rasterizar bytes não confiáveis: `data:` URI evita o *canvas tainting* de `file://` | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| O composer habilita antes de `ai:models` resolver — todo gate de capacidade lê `model === null` por ~5 s | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| O hash de anexo rasterizado é da saída do Chromium; trocar de Electron troca o encoder | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Imagem 800×460 = 270 tokens e ~78 s de prefill nesta CPU | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| `nativeImage` não decodifica SVG | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| O rasterizador já é, tecnicamente, a "segunda janela" — gatilho de `job:event` | [`ROADMAP § 2`](../../ROADMAP.md) |
| Decisões D17.1–D17.13 | [`DECISOES.md`](../../DECISOES.md) |

⚠️ **Lacunas registradas como tais, não como resolvidas:**
- **Cancelar um `attachImage` a meio da rasterização ainda armazena o blob** — mesma lacuna que já existia no caminho PNG/JPEG direto, só alargada. Fora do aceite do passo 7; o GC de órfãos varre depois.
- **`chatMessageSchema.role` aceita `system`, `messageRoleSchema` não** — só importa se o *tool calling* dos planos 21–23 quiser prompt de sistema. Sem gatilho formal.
- **`EBUSY` ao substituir `app.asar` no `build:win`** — reiniciar resolveu, mas nenhum processo foi encontrado segurando o arquivo (`tasklist` e `Get-Process`). **Causa não diagnosticada**, por isso não subiu como armadilha: haveria o conserto sem o mecanismo.

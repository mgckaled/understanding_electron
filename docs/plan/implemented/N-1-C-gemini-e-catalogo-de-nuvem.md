...
|---|---|---|---|
| 25/08/2026 | fechamento pós-commit | 3 furos do `advisor`, corrigidos e verificados ao vivo (`4110b8e`) | O advisor achou exatamente o que o diário já nomeara como não-verificado — **a linha "não foi re-verificada ao vivo" era o furo, não uma nota inofensiva**. O bloqueio de nível 3 com imagem real só foi visto na tela nessa passada; e a linha desabilitada do popover nunca tinha renderizado, porque as três verificações anteriores sempre semeavam as duas chaves. |
| 25/08/2026 | 1–8 | plano executado por inteiro, um commit por passo | Passos 5–6 se fundiram: não dava para verificar o adaptador sem o resolver wireado (mesmo padrão que o N-1-B já registrara). **A verificação ao vivo achou o que a pesquisa não previu:** `thinkingLevel` no nível mínimo é rejeitado com HTTP 400 por `gemini-3.7-flash` — só `gemini-3.1-flash-lite` documenta os quatro níveis. Trocado para o nível mais baixo válido nos dois. Passo 7 estourou o teto de 400 linhas do `ConversationView.tsx` e foi dividido na hora (`useCloudCatalog.ts`), não adiado — o próprio plano já tinha isso como risco nomeado. |
| 25/08/2026 | — | plano escrito | Context7 mudou o desenho: a doc oficial empurra uma API stateful para modelos 3.x, incompatível com o modelo stateless do app — o plano fica deliberadamente no endpoint clássico, com multi-turno como **primeiro sub-passo** do passo 5, não como suposição. Achado ao vivo no meio da sessão: a janela de contexto travava em 32.768 para qualquer modelo de nuvem — `contextCeiling` devolvia nulo para `attention` nulo (correto para "não custa RAM", errado para "que teto a barra oferece"), congelando um número que nunca protegia nada, já que `num_ctx` nunca é enviado à nuvem. |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| O nível mínimo de `thinkingLevel` não vale para todos os modelos Gemini — ficha por modelo | [`reference/models/cloud-optin.md`](../../reference/models/cloud-optin.md) |
| Indexar só o primeiro elemento de um chunk de API externa descarta o resto em silêncio | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| `disabled:text-*` não alcança um descendente com cor própria (`CapabilityChip`) | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Decisões DN1C.1 em diante | [`DECISOES.md`](../../DECISOES.md) |

⚠️ **Achados registrados sem ação, de propósito:** `gemini-3.7-flash` devolveu HTTP 503 "high demand" duas vezes e um timeout na mesma sessão em que `gemini-3.5-flash-lite` respondeu de primeira, com chamada idêntica — provedor sob carga, não defeito do adaptador. E a presença das chaves de API no processo lançado foi **não-determinística** entre lançamentos sucessivos do Electron; não investigado, por estar fora do que a regra de nunca ler `.env` autoriza examinar.

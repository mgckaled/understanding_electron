...
|---|---|---|---|
| 22/08/2026 | todos | **concluído** — plano fecha e move para `implemented/` | Primeiro plano da trilha N. O segredo é de mão única desde o desenho: o renderer grava e pergunta se existe, **nunca lê** — `secrets:read` não existe por decisão (DN1A.3), não por omissão. `Record<AiService, string>` exaustivo reprovou o typecheck assim que o enum ganhou um serviço novo, o que fez `preload`/`api-mock.ts` aparecerem sozinhos como pontos a tocar — a lição que o N-1-B reaproveitou para mover uma tarefa de passo. |

**O que este plano deixou fora dele:**

| Achado | Dono |
|---|---|
| `safeStorage.encryptString` passado como referência solta lança "Illegal invocation" | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| `loadEnv` do electron-vite é tempo de build; `process.loadEnvFile()` é runtime — não intercambiáveis | [`ARMADILHAS.md`](../../ARMADILHAS.md) |
| Segredo é de mão única — o renderer escreve, nunca lê | [`HISTORY.md`](../../HISTORY.md) + [`CLAUDE.md`](../../../CLAUDE.md) |
| Decisões DN1A.1–DN1A.5 | [`DECISOES.md`](../../DECISOES.md) |
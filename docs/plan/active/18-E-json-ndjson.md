...
|---|---|---|---|
| 21/08/2026 | 1-3 | passos 1-3 implementados e commitados; passo 4 em andamento | `sniffDatasetFormat` (passo 1), generalização de `buildViewSqlInterpolated`/`ensureDatasetView` + `core/duckdb/schema.ts` (passo 2), canal `kind: 'schema'` no worker (passo 3) — três commits. Passo 3 verificado contra DuckDB real, sem Electron, num teste permanente (`workers/duckdb/jsonSchema.test.ts`, mesmo padrão de `encodingFallback.test.ts`/`csvSniffAgreement.test.ts`), não por `pnpm dev` isolado: o canal `schema` não tem entrada própria na UI antes de o passo 4 existir (`attachDataset` é quem o chama), então o terceiro item do aceite do passo 3 (`formatByHash` não sondando de novo entre `schema`→`query`/`profile`) fica para a sessão live do passo 5, quando o fluxo completo já existir — adaptação registrada aqui, não pulada em silêncio. Consulta ao advisor no meio da implementação (achados aplicados antes de seguir para o passo 4, ambos com teste real contra o motor, não suposição): **(1)** o teste real inicial só cobria `DESCRIBE`/`COUNT`, nunca o caminho `getColumnsObject → normalizeColumns → columnsToArrowBytes → tableFromIPC` — sondado ao vivo, uma coluna TIMESTAMP de `read_json_auto` chega como `DuckDBTimestampValue` (`{ micros: bigint }`), tipo que `normalizeColumns.ts` (18-B, escrito só contra `DuckDBDateValue`) nunca tratava; sem a correção, o valor chegaria intacto ao `apache-arrow` como objeto opaco, silenciosamente errado — teste construído para falhar antes da correção (revertido e confirmado vermelho), corrigido depois, `DuckDBTimestampTZValue` tratado igual por ter a mesma forma; **(2)** `hasNestedType` só cobria `STRUCT`/`MAP`/`[]` — sondado ao vivo (Risco 3 do plano, antes só hipotético), `read_json_auto` usa o tipo exato `JSON` como fallback quando o tipo de um campo diverge entre linhas, inclusive quando uma delas é um objeto de verdade (que chega serializado como string, não como `DuckDBStructValue`) — `hasNestedType` ampliado para recusar esse tipo também, com teste real cobrindo o caso do objeto escondido atrás do `JSON`. O advisor também apontou (aplicado adiante, no passo 4): `datasetPartSchema` ganhando `format` obrigatório não quebra leitura de conversas antigas porque `conversation:messages` nunca valida `parts` com zod na leitura (`JSON.parse(...) as MessagePart[]`, achado confirmado no código real) — mas a linha condicional do passo 5 precisa checar `format === 'json'` (não `=== 'delimited'`), ou todo anexo CSV já persistido (sem o campo) cairia no ramo errado da UI. |
| 21/08/2026 | — | plano escrito, revisado pelo advisor, ainda não executado | Escrito depois de ler os quatro sub-planos anteriores **implementados** (código real: `core/duckdb/{config,query,profile,protocol}.ts`, `workers/duckdb/index.ts`, `main/duckdb/spawnWorker.ts`, `main/features/dataset/handlers.ts`, `main/attachments/{storage,gc}.ts`, `AttachButton.tsx`/`DatasetCard.tsx`/`DatasetPreview.tsx`/`DatasetQueryPanel.tsx`/`DatasetProfile.tsx`), não dos planos em si. Context7 consultado (`duckdb-web`) para `read_json_auto`: confirmou detecção automática `auto`/`unstructured`/`newline_delimited`/`array` num só parâmetro `format` (por isso `DatasetSummary` não precisa distinguir JSON de NDJSON, só `'json'`), e confirmou que JSON aninhado **não** produz erro — infere `STRUCT`/`MAP`, o motivo real da D18E.4. Decisão central, batida com o advisor **antes** de escrever o plano inteiro (chamado uma vez, no meio da fase de desenho, não só ao final): detectar formato pelo conteúdo do arquivo, com uma função só rodando dos dois lados (main no anexo, worker na consulta), em vez de estender `dataset:query`/`dataset:profile` com um parâmetro novo — evitando de propósito o mesmo erro que o advisor corrigiu no 18-C (parâmetro novo num canal fechado, quando existia caminho mais barato). O advisor confirmou a escolha e apontou dois achados aplicados antes de escrever os passos: (1) BOM (U+FEFF) sobrevive ao `StringDecoder`/`trimStart()` e faria todo JSON com BOM cair no leitor de CSV em silêncio — corrigido em `sniffDatasetFormat`, com fixture própria no passo 1; (2) a ordem de `attachDataset` para JSON (hash → guarda → pergunta esquema ao motor) deixa um blob copiado se `runSchema` rejeitar — verificado contra `gc.ts` real (não presumido) que `collectOrphanedAttachments` já cobre exatamente esse caso, por já varrer todo hash não referenciado por mensagem nenhuma, na inicialização. Dois limites de escopo reafirmados por decisão explícita, não por esquecimento: `core/dataset/scan.ts` (CSV) não migra para DuckDB, mesmo tendo acabado de ensinar o app a pedir esquema ao motor para JSON — unificação já adiada desde o 18-C (D18C.7); JSON aninhado é **recusado**, não achatado — fora de escopo nomeado no próprio `ESCOPO.md`. Achado do advisor sobre o passo 5, aplicado: "os quatro componentes não mudam" não é o mesmo que "está provado" — `read_json_auto` pode produzir `TIMESTAMP` de uma string ISO, tipo que `normalizeColumns.ts` (acrescentado no 18-B especificamente para o `DuckDBDateValue` do CSV) nunca viu vindo de outro lugar; o passo 5 nomeia isso como o item real a verificar, com um fixture cobrindo os seis tipos de `arrow.test.ts` mais `NULL`, não uma verificação genérica de "funciona". Seis passos, dentro do teto de sete — o advisor confirmou que não há uma medição dedicada a fazer (diferente do 18-D, o custo do anexo de JSON acontece uma vez, sem uma alternativa real para comparar). **Submissão do plano completo ao advisor, dois achados aplicados antes de fechar**: D18E.2 original transformava `DatasetSummary` numa união discriminada por `format`, o que obrigaria `scanDelimited` a passar a emitir esse campo — contradizendo a própria D18E.5 ("`scan.ts` fica intocado"); corrigido para `DatasetSummary` continuar exatamente como está, e `format` ser montado só em `attachDataset`, na hora de construir o `DatasetPart` final. O passo 5 listava "`DATE`/`TIMESTAMP`" como um único item do fixture — o risco real é especificamente `TIMESTAMP` vindo de string ISO (tipo que `normalizeColumns.ts` nunca viu, escrito contra o que `read_csv_auto` produz), e `DATE` sozinha não o exercitaria; corrigido para exigir as duas colunas separadas no fixture do passo 5. |

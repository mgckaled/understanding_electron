...
`extractDomainId(args: unknown): string | null` (DO6.3), nível 1, sem I/O. `createIpcStatsStore` ganha `setEventSink` (DO6.2) e `record()` passa a chamar o sink com `{ channel, durationMs, error, domainId: extractDomainId(args) }`, sem mudar `AppIpcStat`/`snapshot()`. Teste de nível 1: sink chamado uma vez por chamada concluída (sucesso e erro), nunca por chamada em voo; `domainId` correto para um `args` com `conversationId`, outro só com `jobId`, outro sem nenhum; sink ausente (`setEventSink` nunca chamado) não lança.

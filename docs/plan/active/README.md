...
| [**DS-5**](DS-5-icones-fonte-e-acabamento.md) | 🚧 **Ícones, fonte de código, realce de sintaxe e o acabamento que o DS-4 não pediu** — escrito em ago/2026, terceira rodada de handoff da trilha. Biblioteca de ícones Lucide substituindo **todo** glyph Unicode do app (DS5.1); `--font-mono` inteiro vira JetBrains Mono auto-hospedada (DS5.2); diagnóstico do realce de sintaxe a partir de hipótese nula, não de defeito presumido (DS5.4); escala tipográfica do chrome; clipe no composer relocando `OpenDatasetPanel` da sidebar; seletor de modelo dividido em pílula de modelo + pílula de contexto (DS5.6); botão de copiar funcional no bloco de código e no turno do assistente, com compartilhar/atualizar desabilitados por ora (DS5.7) | **9 fases — comparado contra cinco imagens em `notes/`** |

**Por que três, depois quatro, e agora cinco.** Os dois primeiros compartilham o aceite mais forte que existe para migração — *se a tela mudou, algo saiu errado* —, verificável em segundos. O DS-3, o DS-4 e o DS-5 têm o aceite oposto: a tela muda, e a mudança é a entrega, medida contra um protótipo ou referência visual. Cada um nasceu de uma rodada de handoff escrita **depois** da anterior fechar — não é uma divisão do mesmo escopo, é uma nova rodada a cada vez, e nenhuma delas se declara "definitivamente" a última sem risco de ser desmentida pela próxima (a DS-4 se disse assim; a DS-5 evita repetir a frase).

...
|---|---|---|
| **13** | **Casca do aplicativo.** Layout de duas colunas, sidebar em três regiões por slot (nav · conteúdo · rodapé), conversa em altura cheia, composer fixo. Tudo em memória — cria conversa, troca entre elas, some ao fechar. Zero canal de IPC novo. **Aceite:** a casca abriga **duas** coisas diferentes no mesmo slot — nav com *Conversas* e *Configurações*, esta última uma tela vazia. Casca que só provou abrigar uma coisa é indistinguível de casca que a fixa. | Onde mora o estado compartilhado; o que acontece com `OpenDatasetPanel` (preso por e2e ao rótulo "Escolher arquivo") e `Versions`; se `features/ai-chat/` vira `features/conversation/` |

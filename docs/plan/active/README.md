...
- **Trilha O (observatório)**: [~~O-1~~](../implemented/O-1-a-casca-do-observatorio.md), [~~O-2~~](../implemented/O-2-ipc-jobs-e-fila-do-worker.md), [~~O-3~~](../implemented/O-3-os-dois-motores-se-descrevem.md), [~~O-4~~](../implemented/O-4-capacidades-o-primeiro-painel-caro.md), [~~O-5~~](../implemented/O-5-uso-de-disco-e-cache-do-chromium.md) e [~~O-6~~](../implemented/O-6-observatoriodb-e-fluxo-de-eventos.md) ✅ já implementados — o O-6, sexto plano, é o primeiro que grava (nasce `observatory.db`). **[O-7](O-7-desempenho-por-modelo.md) tem arquivo em `active/`** desde 01/09/2026 (desempenho por modelo, ainda não implementado); O-8 (livro-razão de privacidade) segue sem arquivo — a trilha é **gatilhada, não sequencial**, e cada painel entra quando o que ele observa passa a existir. A fundamentação — os seis eixos, o inventário classificado por custo/trabalho/situação, o critério `crivo.db` vs. `observatory.db`, e o desenho fechado de O-7/O-8 (§ 9) — é de [`reference/observatory/`](../../reference/observatory/README.md); a ordem dos oito está no [`ROADMAP § 1`](../../ROADMAP.md#1-a-sequência).

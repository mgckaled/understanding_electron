...
| [**DS-3**](DS-3-a-interface-chega-ao-alvo.md) | ← **próximo a executar** (escrito). **A interface chega ao alvo.** Não são ajustes: cabeçalho e rodapé da sidebar, busca por título, agrupamento por data, barra de acento na conversa ativa, título da conversa como cabeçalho, desmonte da toolbar superior, seletor de modelo como pílula dentro do composer, envio circular e pausa, bolha na mensagem do usuário, escala de tipo (só a de leitura — DS3.2). Os estados de `alvo/` **não** contribuem: são o modal de Configurações, com diferenças recusadas ou de outro dono (veredito no plano) | **A tela muda, e a mudança é a entrega** — medida contra `alvo-chat.png` |
...
**O DS-3 ainda não está escrito**, pela regra do próprio arco: um plano nasce quando é o próximo a executar. A linha dele acima é o contrato, e o alvo é o aceite. O poder de veto que o passo 0 do DS-1 tinha sobre os dois **não foi exercido** — os dois riscos caíram, e a forma da solução se manteve.

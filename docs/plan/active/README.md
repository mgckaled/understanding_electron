...
| [~~**DS-6**~~](../implemented/DS-6-fundacao-desktop-robusta.md) | ✅ **Fundação de desktop robusta em `base.css`/`tailwind.css`** — concluída em ago/2026, a partir de relatório externo avaliado item a item contra o repositório real (`advisor` Opus como filtro). `color-scheme` por tema, `forced-colors` (Windows High Contrast), `overflow-wrap`, autofill do Chromium neutralizado com o token real do campo, `caret-color`, `user-drag` desligado em mídia, `img` normalizado, `::selection`; convenções documentadas sem tocar CSS (`tabular-nums`, tokens fora de `@theme inline`, `:disabled`); toda a narrativa de decisão que vivia em comentário nos dois arquivos migrou para a skill `design-system`, com só o aviso catastrófico (DS1.5) sobrevivendo no fonte; `guard.mjs` passou a cobrir `base.css`. Verificada ao vivo (Playwright temporário, escrito e apagado) contra o app real: `color-scheme`, `::selection` e `forced-colors` nos dois temas. Entrada em [`HISTORY.md`](../../HISTORY.md) | **Nenhuma mudança de layout — só robustez de plataforma, verificada ao vivo** |
| [~~**DS-7**~~](../implemented/DS-7-consolidacao-de-tokens-css.md) | ✅ **Consolidação de `tokens.css`** — concluída em ago/2026, a partir de relatório externo avaliado item a item (`advisor` Opus como filtro). `tokens.css` enxuga para declaração pura, citação única à skill no cabeçalho; a skill ganha convenção de hover/seleção já existente (sem tint), regra de radius corrigida contra o uso real, ausências e convenções registradas, detalhe de `--color-backdrop`/`--syntax-*`, e duas contagens que tinham envelhecido corrigidas (`--font-size-sm` 14px, não 13px; layout da casca com um consumidor real, não três). `ROADMAP.md` ganha o gatilho do Primer. Recusou os dois itens que trariam token novo (superfície interativa, radius de badge) contra o código real. Entrada em [`HISTORY.md`](../../HISTORY.md) | **Nenhum token novo, nenhum pixel muda — sem verificação ao vivo** |
| [~~**DS-8**~~](../implemented/DS-8-primitivos-refino-e-limpeza.md) | ✅ **Primitivos: refino de acessibilidade, limpeza de órfãos** — concluída em ago/2026, a partir de relatório externo avaliado item a item (`advisor` Opus como filtro, chamado três vezes). `Toolbar` e `Panel` apagados — zero consumidores, confirmado por `git log`/grep no repositório inteiro, achado que o relatório não tinha (primitivos: nove → sete). `Button` mantém nome acessível durante `loading`; `Field` injeta `aria-invalid`; `Dialog` rola conteúdo longo e aceita `describedBy`; `Popover` ganha `className` interno e os 5 gatilhos reais ganham `aria-haspopup`/`aria-expanded`; `Slider` comita pelo evento nativo `change` (fim do binding duplicado) e ganha `aria-valuetext`. `cx()` zero-dep substitui o join manual em três primitivos — `clsx`/`tailwind-merge` recusados. Verificação ao vivo achou uma regressão real (`Dialog`'s `flex` sem escopo `[open]` derrotando o UA stylesheet, mesma armadilha do `Popover`) antes do fechamento. Entrada em [`HISTORY.md`](../../HISTORY.md) | **`Dialog` rola de verdade — achado e corrigido só na verificação ao vivo** |
...
| [**19**](19-propor-consulta-e-passos.md) | **Propor: consulta e passos.** `core/pipeline/`, o schema zod alimentando `format` e `.parse()`, a união discriminada `query \| steps`, e a verificação pós-execução. | Onde a lista de passos aparece: dentro da mensagem do assistente, ou numa região própria |

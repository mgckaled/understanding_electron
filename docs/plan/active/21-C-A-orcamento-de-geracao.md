...
- **D21C.12 — o `promptTokens`/`evalTokens` real de cada turno é persistido, não descartado após calibrar.** Migração `v5` (`prompt_tokens`/`eval_tokens` em `messages`), campos espelhados em `messageSchema`; `MessageList.tsx` exibe uma legenda ("Prompt: N · Geração (raciocínio + resposta): M tokens") sob toda resposta que carrega o dado — o único lugar onde o custo real de um turno com raciocínio pesado fica visível, já que o medidor nunca reenvia raciocínio (D21A.3) e não tem como estimá-lo.
- **D21C.13 — ancoramento pós-fato: `budgetFor` estima só os chars somados desde a última medição real**, não a história inteira pela razão a cada chamada (o "N+1 não acumula" relatado ao vivo). `anchor?: {tokens, chars}` em `budgetFor`; `useConversationChat.ts` guarda `{tokens, chars, imageCount}` do último turno assentado. Dois cuidados que a primeira versão não tinha, achados pelo advisor: (a) o anchor precisa **hidratar** ao abrir uma conversa já existente — `anchorFromHistory()` (`core/ai/messages.ts`) reconstrói a partir da última mensagem `assistant` com `promptTokens`, senão reabrir uma conversa volta a mostrar a contagem linear até o próximo envio; (b) `removeMessage` pode encolher o histórico abaixo do ponto do anchor — `budgetFor` ignora o anchor (volta a estimar a história inteira) quando `historyChars < anchor.chars`, senão a estimativa trava inflada e pode recusar um envio legítimo.
...
|---|---|
| 1 (03/09/2026) | Sondagem inicial do arco 21-C (três frentes), revisão do advisor (Opus) split o plano em três arquivos e corrigiu o timing da EMA e o gate por `costed`. Implementação completa nesta mesma sessão: `budgetFor`/`calibrateRatio` em `core/ai/budget.ts`, `Composer.tsx`/`ConversationView.tsx`/`useConversationChat.ts` ajustados, testes de nível 1 e 2 novos. Dois bugs pegos pelos próprios testes durante a implementação (state inicial do `charsPerToken` nascendo `DEFAULT_CHARS_PER_TOKEN` em vez de `undefined`, e o fallback esquecido no retorno do hook) — ambos corrigidos. Segunda revisão do advisor, sobre a implementação já pronta, achou um terceiro bug real: o hook não desmonta ao trocar de conversa, então a razão calibrada de A vazava para B (decaimento 0,6ⁿ por turno) — corrigido com o mesmo padrão de comparação em tempo de render do `prevThinking` do 21-B, teste novo em `ConversationView.test.tsx` prova o não-vazamento. `pnpm check:fast` verde (1335/1335). **Verificação ao vivo pendente** — fica com o usuário. |
| 2 (03/09/2026) | Verificação ao vivo da sessão 1 (Ollama, `qwen3.5:2b`): faixas fixas + campo numérico aprovados (21-C-C); o medidor de tokens reportado como "provavelmente errado" — 289 tokens de prompt (pergunta+imagem), 502 de geração (raciocínio muito mais longo que a resposta final), números reais nunca exibidos. Diagnóstico confirmou dois defeitos: (1) raciocínio nunca contado, por desenho (D21A.3); (2) a razão era reaplicada sobre a história INTEIRA a cada render, não ancorada no que já foi medido de verdade ("N+1 não acumula", relatado pelo usuário). Correção pedida em conjunto: **D21C.12** (persistir `promptTokens`/`evalTokens` reais — migração `v5`, legenda visível em `MessageList.tsx`) e **D21C.13** (ancoramento pós-fato em `budgetFor`). Revisão do advisor sobre a implementação achou 3 problemas antes do commit: anchor nunca hidratado ao reabrir uma conversa (bloqueante — corrigido com `anchorFromHistory()`), anchor obsoleto quando `removeMessage` encolhe a história abaixo do ponto ancorado (corrigido com fallback em `budgetFor`), e ausência de red-check no teste de vazamento entre conversas. Red-check aplicado (skill `testing`): nem a linha de reset nem o galho de hidratação isolados derrubam o teste sozinhos — os dois cobrem a mesma lacuna por lados diferentes, quando a conversa nova ainda não carregou mensagens — teste renomeado para provar o que ele prova de fato (a estimativa de B vem só da própria história de B, sem herdar os números de A), não o mecanismo específico. `pnpm check:fast` verde (1351/1351). **Verificação ao vivo desta correção pendente** — fica com o usuário. |

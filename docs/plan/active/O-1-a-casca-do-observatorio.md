...
| 29/08/2026 | 1–4    | passos 1–4 entregues; **falta o passo 5 (conferência ao vivo, do usuário)** | `pnpm check:fast` verde, 1146 testes. Quatro achados que o plano não previa. (1) **`residentSet` não existe** — o campo é `workingSetSize`; a DO1.7 continua certa (o C++ escreve `>> 10`), o nome no sketch não. (2) **`type` virou união** em vez de `string` + comentário listando valores, e o painel ganhou `Record` exaustivo. (3) **`utilityProcess.fork` sem `serviceName` reporta `Node Utility Process`** — sem uma opção no fork, a asserção de nível 2 do passo 4 não teria como existir. (4) **A provocação (c) do plano dá falso negativo**: importar `electron` por valor não lança, e a suíte fica verde com o defeito; o que reprova é `Function.length` (subiu para `ARMADILHAS.md`). Duas medições: o chunk tardio corta **70 bytes** do bundle inicial (3.308,84 → 3.308,77 kB, mais 3,19 kB de chunk) — o corte é real mas hoje é investimento, não economia; e `formatSize` já tinha 3 consumidores e 2 cópias, então ganhou dono em `shared/format.ts` antes de o painel virar o quarto. A DO1.4 foi executada: as versões saíram de Configurações. |

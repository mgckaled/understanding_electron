...
Confirmado ao vivo, 2/2 (`gemini-3.5-flash-lite` e `gemini-3.7-flash`): nenhum `part.thought` chega, em nenhum dos dois. Não é o risco intermitente do achado #2 do guia — é mais estrutural. A documentação atual do Google (`ai.google.dev/gemini-api/docs/thinking-mode`, via Context7) moveu todo exemplo de raciocínio visível para a **Interactions API** (`client.interactions.create`), com a frase explícita: "the generateContent API does not provide dedicated thought blocks, instead attaching signatures as metadata". Corroborado por relatos externos desde jan/2025 (`discuss.ai.google.dev`, issues no `litellm`/`python-genai`) de que `streamGenerateContent` parou de devolver `thought`. **Decisão, não bug a corrigir agora:** o código já degrada sem quebrar (nenhum `reasoning` no retorno quando `reasoningAssembled` fica vazio, D21A.3) — 21-A fecha assim, documentado como limitação conhecida do provedor. Migrar para a Interactions API é item do `ROADMAP § 2`, condicionado a confirmar se `store: false` preserva o modelo stateless full-history-resend que D21A.6 assumiu impossível.

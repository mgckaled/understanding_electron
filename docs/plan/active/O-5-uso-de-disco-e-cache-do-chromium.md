...
### DO5.9 — Os dois painéis Caro/Acessível vivem no cache do `QueryClient`, nunca só em `useState`

Achado numa revisão do advisor Opus **depois** da primeira implementação: a primeira versão de `DiskUsagePanel` guardava o resultado em `useAsyncAction` (local, `useState`) — e a § 4.2 desmonta o painel inativo, então trocar para "Banco de dados" e voltar para "Uso de disco" perdia a medição inteira, voltando ao botão inicial como se nada tivesse rodado. `CapabilitiesPanel`/`useCapabilities` nunca tiveram esse problema porque guardam o resultado numa `useQuery({ enabled: false })` — o `QueryClient` sobrevive ao desmonte do componente, só a assinatura de `useJobProgress`/o `jobId` do botão "Cancelar" precisam ser locais. `DiskUsagePanel` foi reescrito no mesmo molde. Efeito colateral corrigido junto: `ChromiumCachePanel` tinha um `staleTime: 60_000` inventado — o `createQueryClient()` já define `staleTime: Infinity` global (nada fica velho sozinho, só invalidação explícita refaz a leitura), e um TTL de 60s é mais fraco que "cache obrigatório" (§ 5.1), não mais forte. Removido; o painel agora herda o default.
...
- `ChromiumCachePanel.tsx` — `useQuery(['session','cacheSize'], ...)` **sem** `staleTime` próprio (DO5.9: herda o `Infinity` global de `createQueryClient()` — cache obrigatório de verdade, não um TTL de 60s inventado), `Row` com `formatBytes`, botão "Limpar cache" (`useMutation` → `clearCache` → invalida a query), reaproveita `StateView`.
- `DiskUsagePanel.tsx` — mesmo padrão `enabled: false` + `refetch()` do `useCapabilities` (DO5.9), não `useAsyncAction`/`useState` local: o resultado e o `dataUpdatedAt` vivem no `QueryClient`, sobrevivendo à troca de painel (§ 4.2 desmonta o painel inativo). `jobId` continua em `useState` só para alimentar `useJobProgress`/o botão "Cancelar" enquanto o painel está montado; ao concluir, tabela `crivo` (nome/tamanho/aviso se `partial`), linha "Chromium (motor embutido)" com `runtimeBytes` (+ aviso se `runtimePartial`), total, e "Medido há Xmin · ↻" via `formatAge`.
...
- Nível 3: `src/main/features/session/handlers.test.ts`, `src/main/features/disk/handlers.test.ts` — injeção de `getCacheSize`/`clear` fake. **Sem** o teste de `Function.length` que o O-1 introduziu (skill `testing` § *O nível 3 é o que quase ninguém tem*): ele prova algo só quando o handler tem um parâmetro com **default** que poderia esconder um `require('electron')`; nenhum dos dois handlers daqui tem default nenhum, então o teste seria vacuamente verdadeiro — decisão tomada ao escrever `session/handlers.test.ts`, não um esquecimento.
...
| 01/09/2026 | 3 | Conferência ao vivo do usuário: os dois painéis funcionaram de primeira (prints anexos), sem `partial`. Segunda rodada do advisor Opus (a primeira tinha caído por sobrecarga) achou um defeito real antes do fechamento: `DiskUsagePanel` perdia a medição inteira ao trocar de painel e voltar (§ 4.2 desmonta o inativo, e o resultado vivia só em `useState`) — reescrito no molde `useQuery({ enabled: false })` de `useCapabilities` (DO5.9), com teste de regressão cobrindo remount. Corrigido junto um `staleTime: 60_000` inventado em `ChromiumCachePanel` (o `createQueryClient()` já garante `Infinity` global — o TTL de 60s era mais fraco que "cache obrigatório", não mais forte). Também corrigida a alegação de `Function.length` na Verificação (nunca escrita nos testes, por ser vacuamente verdadeira aqui). `check:fast` verde de novo depois das correções. Fechando o plano: README raiz ganhou nota sobre `%APPDATA%`/`userData`/anexos (pedido à parte do usuário), `HISTORY.md` ganha a entrada do marco, plano move para `implemented/`. |

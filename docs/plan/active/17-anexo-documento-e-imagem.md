...
>
> **Para retomar (passo 7 — SVG/WebP → PNG):** spike dos três candidatos da D17.7 primeiro (nenhum código de extrator antes de escolher). `core/image/sniff.ts` (passo 5) só reconhece PNG/JPEG hoje — o rasterizador é quem produz o PNG que ele já sabe validar, então o sniff não muda. Sem cobertura abaixo do nível 4 (risco 2 do plano, propriedade genuína).

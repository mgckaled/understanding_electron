...
| **5** | Mecanismo de imagem, invisível: `imagePartSchema` + `image:pick`/`image:attach` + extrator PNG/JPEG (passagem direta) + materializador assíncrono de `images` (fecha D17.5) + protocolo `attachment://` + CSP (D17.6) + `join('\n\n')` passa a filtrar contribuição vazia (movido do passo 4: só ganha sentido/consumidor quando uma parte `image` de fato contribui `''`) | Nível 1 (materializador com resolver stub, validação de hash), nível 3 (handler) | PNG/JPEG anexados viram `MessagePart` com hash correto; `attachment://<hash>` serve bytes com Content-Type correto; hash inválido devolve 400 |

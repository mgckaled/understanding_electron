...
**Entrega:** `.claude/skills/ai/SKILL.md` nascido, dono de "o que decide a primeira linha ao tocar código de IA"; `CLAUDE.md` aliviado das regras de escolha de modelo e do protocolo de sonda do Ollama; `docs/reference/reasoning/README.md` auditado e, confirmando o achado desta sessão de planejamento, marcado `⛔ consumido`; as seis skills existentes e as duas tabelas de fonte única (`CLAUDE.md`, `docs/README.md`) ganhando ponteiro para a skill nova; `ROADMAP.md` registrando a compressão residual como **R-7**, ainda sem arquivo.

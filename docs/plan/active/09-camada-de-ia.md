...
|---|---|
| Fronteira de rede/modelo injetável e testável sem Ollama | D2 da [visão geral](00-visao-geral.md) — DIP como parâmetro de função; `core/` puro |
| Falha esperada e acionável (serviço fora do ar, chave ausente, limite de taxa) | D2.2 da [fase 02](02-contrato-ipc.md) — `Result` com `AppError` |
...
| Resposta em fluxo, token a token | Variante `chunk` de `JobEvent`, [fase 02](02-contrato-ipc.md) |

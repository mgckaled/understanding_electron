...
|---|---|---|---|
| 18/08/2026 | 1 | Passos 0-7 completos, um commit cada: `c2be3d2` (0, plano+ROADMAP), `d4af098` (1, checklist auto-conservação no CLAUDE.md), `b5b8528` (2, docs/README.md), `08eb867` (3, README.md raiz), `39dad38` (4, HISTORY-archive.md nasce + 11 marcos da fundação), `b77810a` (5, trilha DS — 6 marcos), `21a8354` (6, trilhas R+F — 2 marcos), `dd6cfb0` (7, 13 armadilhas de trilha fechada relocadas — corrigido no processo um lapso de heading sem sufixo "(ago/2026)"). Passo 8 (comprimir as 35 armadilhas que ficam — o agente de design estimou 36, a contagem real após o passo 7 é 35) iniciado e parado num checkpoint intermediário: `e6025e7` comprime as **14 primeiras**, na ordem em que aparecem no arquivo — Rasterizar bytes/`data:` URI · `crypto.Hash` passthrough · Decodificar chunk UTF-8 · `animationiteration` 14 filhos · jsdom esconde popover · Flexibilidade sem saída (D15.13) · razão caracteres-por-token · razão que se cancela (D15.14) · Travar invalida texto (D15.13) · Coleção vazia assíncrona · Margem absoluta (D15.10) · `/api/tags` omite `vision` · Janela deslizante 30× · Ollama descarta prompt em silêncio. | **Retomar no meio do Passo 8**, comprimindo pela mesma régua (número medido + mecanismo + conserto, narrativa de investigação sai) as **21 armadilhas restantes, nesta ordem exata no arquivo**: Um controle que copia o valor no `mount` · Teste de nível 2 clica em botão que o CSS esconde · O modelo descreve uma imagem que nunca recebeu (tem bloco de código, preservar) · `/api/embed` aceita `images` e ignora · O anexo custa ~80s uma vez · Três formatos de imagem que deveriam funcionar · O evento `scroll` é assíncrono (citada por skill `testing` — poda leve) · jsdom não implementa `<dialog>` (citada — poda leve) · Playwright emula `prefers-color-scheme: light` (citada — poda leve) · 485 avisos de lint/`core.autocrlf` · NL→SQL sobre dado sujo (tem bloco SQL, preservar; muito citada por outras entradas — manter heading intacto) · opção `languages` do `rehype-highlight` · tema `github.css` desatualizado · Token de cor de estado (D10.1) · `APP_ID` dois propósitos · `app.asar` empacotava `.claude/settings.local.json` (tem parágrafo de "Continuação" — preservar os dois) · Import de `electron` quebra em teste (citada — poda leve) · Glob de `coverage.include` (citada por 2 skills — poda leve) · `ArrayBuffer` transferível não é posse · Preload sandboxed quebra com `zod` · `rl.close()` não destrói stream (citada pela skill `architecture` — poda leve). Depois do Passo 8: passos 9-15 inteiros ainda pendentes (comprimir os 17 marcos que ficam em dois lotes, as 4 skills, e o fechamento) — passo a passo completo em `docs/plan/active/R-2-documentacao-tecnica-e-historica.md` acima. **Regra que não pode ser esquecida ao retomar:** nunca reescrever texto de heading `###`, só o corpo; conferir `git diff --stat -- src/` vazio a cada commit; o Passo 1 (checklist de auto-conservação) já está no `CLAUDE.md` e pode ser usado como exemplo de regra (a) vs (b). |

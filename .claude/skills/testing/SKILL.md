...
- Um teste de "a lista sobrevive a fechar e reabrir o painel" passava com o estado de volta **dentro** do painel — o desenho que ele existia para reprovar. A causa não é o jsdom: `DocsPanel` **devolve `null`** quando não há o que mostrar, e devolver `null` **não desmonta** — o fiber segue vivo e os hooks guardam tudo. Nenhum teste de preservação distingue os dois desenhos quando o componente nunca sai da árvore. ⚠️ E há uma **segunda** causa que mascara a primeira: fechar só marca `data-closing`, então reabrir no clique seguinte nem exercita a saída (`await waitFor(… not.toBeInTheDocument())` entre os dois) — consertar só essa deixa o teste igualmente vazio. **Antes de afirmar "o estado sobrevive a X", verifique se quem o guarda realmente desmonta em X.**
...
⚠️ **Teste que passa isolado e estoura por timeout na suíte inteira não é intermitência — é custo de render.** O teto padrão do Vitest é 5 s e a suíte roda com `maxWorkers: '50%'`, então um teste a ~2,2 s isolado já vive a ~2,3× do teto sob contenção: qualquer coisa que some ~10% ao render o empurra para fora. **Quem pega isso é o portão de commit, nunca o `test_related`**, que roda o grafo do arquivo tocado sem contenção e fica verde. Os dois consertos errados são simétricos — subir o `testTimeout` esconde a próxima regressão, desfazer a mudança joga fora a feature por um sintoma que não é dela. **Meça o delta** rodando o arquivo isolado antes e depois, ataque a causa, e para o que sobrar use um **teto explícito no próprio teste** (`it(nome, fn, 15_000)`) com o número medido em comentário — teto por teste é dimensionamento, teto global é cegueira ([`ARMADILHAS.md`](../../../docs/ARMADILHAS.md)).

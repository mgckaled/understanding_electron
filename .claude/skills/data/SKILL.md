...
`DUCKDB_MEMORY_LIMIT = '2GB'` em `core/duckdb/config.ts` — remedido ao vivo no 18-A (5,54 GB livres de 15,81 GB, sem Ollama residente — orçamento de RAM de modelo é da skill [`ai`](../ai/SKILL.md)), não copiado do `ESCOPO.md`. DuckDB derrama para `temp_directory` acima do limite — errar para baixo é lento, não quebrado. **Não reaproveite este número sem remedir a RAM livre da máquina de novo** — mesmo princípio de [`reference/ambiente/`](../../../docs/reference/ambiente/README.md).
...
A formatação de célula (`∅` para `null`/`undefined`, `.toString()` para `bigint`) tem **um** dono desde o plano 19: `features/attachment/formatCell.ts`. A régua dos três disparou ali, no plano 19 — dois componentes carregavam cópia própria até o terceiro chegar. ⚠️ **Os nomes daquela época já não existem:** hoje os consumidores são as três tabelas do painel de artefato (linhas, resultado de consulta, resultado de passos), depois de F-3-D e F-3-F. **Importe, não copie**; e é um `.ts` avulso, não dobrado no `DatasetTable.tsx`, porque `react-refresh/only-export-components` recusa um `.tsx` exportando algo além de componente.

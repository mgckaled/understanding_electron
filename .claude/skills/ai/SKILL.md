...
⚠️ **A dupla `qwen3.5` tem `attention.head_count_kv: null`, e o motivo é arquitetura: `qwen35` É híbrido Mamba** — publica bloco `ssm.*` completo, como o `granite4`. `readAttention()` não custeia nenhum dos dois, e estes **estão na frota e são os mais usados**. Dono do conserto: **`N-3-A`** (`DNC-31`), que absorveu esta frente; a `F-6` ficou só com o gate `vision`+`tools`.

> 🔍 **Medido em 13/09/2026, e a medição derrubou a hipótese que estava registrada.** Carregar o modelo em dois `num_ctx` e ler o residente em `/api/ps` dá o custo por token **sem precisar de `head_count_kv`**: **36.985 B/token** (`qwen3.5:4b`) e **14.500 B/token** (`2b`). A hipótese de usar `head_count` no lugar preveria **347.341 B/token** para o `4b` — **9,4× para cima**, esmagando o teto para ~1/9 do real. A causa é a hibridez: com `full_attention_interval = 4`, **só 1 em cada 4 camadas cresce**; as demais são Mamba, de estado constante. Com `growingLayers = block_count / full_attention_interval` e os `key_length`/`value_length` publicados, a fórmula fecha — resíduo de **1,13–1,18**, acima do `OVERHEAD` de 1,06 e na direção que **subestima**. ⚠️ `head_count / 4` e `head_count / full_attention_interval` são indistinguíveis nestes dois modelos: o intervalo é 4 nos dois.

⚠️ **E enquanto não corrigido o dano não é de orçamento, é de tela.** `ConversationView.ceilingOf` faz `attention === null ? contextLength : contextCeiling(...)` — ramo escrito para **nuvem** (DN1C.2) em que os dois `qwen3.5`, **locais**, caem. O seletor oferece **262.144** onde cabem 48k (`4b`) e 168k (`2b`), `fitsInMemory` nunca diz `não cabe`, e janela grande demais faz o Ollama descartar o começo do prompt em silêncio. **`attention === null` está sobrecarregado** — significa "é de nuvem" **e** "não consegui ler"; `DNC-31` separa os dois decidindo por serviço.

...
O que decide escolha, em uma linha cada: `gemma3:4b` é o **default**, com visão · `gemma3:1b` é o fallback de baixa RAM · `qwen2.5-coder:3b` é o candidato a default do NL→SQL (único que junta código e folga de RAM) · **a dupla `qwen3.5`** junta `completion`, `vision`, `tools` e `thinking` — as quatro — e é o que mais se usa na prática: `qwen3.5:2b` (2,55 GB) foi o primeiro da frota com as quatro e tem mediana de ~38 tok/s de saída no painel Desempenho do Observatório, contra ~5 tok/s dos modelos maiores; **`qwen3.5:4b` (3,16 GB, sondado em 13/09/2026) é o mesmo modelo mais robusto e ainda cabe nesta máquina** — mesmas quatro capacidades, mesmo teto de 262.144, e é o preferido quando a RAM permite.

⚠️ **`gemma3:4b` não é mais o único com visão — já são três.** Reconfira `capabilities` (`/api/show`) antes de repetir "único com X" em qualquer lugar — a frota muda mais rápido do que os documentos que a descrevem, e `docs/reference/models/` ainda não tem o reinventário completo. Ficha técnica de cada um (peso, teto, KV/token): [`docs/reference/models/`](../../../docs/reference/models/README.md) — esta skill decide qual usar, aquele diretório decide o custo de usá-lo.

⚠️ **A dupla `qwen3.5` tem `attention.head_count_kv: null`, e o motivo é arquitetura: `qwen35` É híbrido Mamba** — publica bloco `ssm.*` completo, como o `granite4`. ✅ **Custeado desde o `N-3-A`** (`DN3A.6`–`DN3A.9`): `readAttention` admite o bloco quando `head_count` e `full_attention_interval` estão publicados no lugar de `head_count_kv`, e a aritmética híbrida tem um dono só (`memory.ts`). A `F-6` encerrou sem virar código: a coexistência de `vision` e `tools` não quebra nada, porque **o gate é por anexo** — imagem pede `vision`, dados e documento viajam como texto e não pedem nada, e `tools` não gateia nada no app.

> 🔍 **Medido em 13/09/2026, e a medição derrubou a hipótese que estava registrada.** Carregar o modelo em dois `num_ctx` e ler o residente em `/api/ps` dá o custo por token **sem precisar de `head_count_kv`**: **36.985 B/token** (`qwen3.5:4b`) e **14.500 B/token** (`2b`). A hipótese de usar `head_count` no lugar preveria **347.341 B/token** para o `4b` — **9,4× para cima**, esmagando o teto para ~1/9 do real. A causa é a hibridez: com `full_attention_interval = 4`, **só 1 em cada 4 camadas cresce**; as demais são Mamba, de estado constante. Com `growingLayers = block_count / full_attention_interval` e os `key_length`/`value_length` publicados, a fórmula fecha — resíduo de **1,13–1,18**, acima do `OVERHEAD` de 1,06 e na direção que **subestima**. ✅ **O confundimento `head_count / 4` contra `head_count / full_attention_interval` está resolvido** — indistinguíveis por medição (o intervalo é 4 nos dois), mas o `config.json` publicado dá `num_key_value_heads` **4** e **2**: é o valor real, omitido pelo GGUF. ⚠️ **A razão Q/KV não é constante de família** (a MoE de 397B usa 16), então `HYBRID_QUERY_PER_KV_HEAD = 4` é o número da linha densa, com gatilho no [`ROADMAP § 2`](../../../docs/ROADMAP.md).

⚠️ **O teto oferecido decide por SERVIÇO, nunca por `attention === null`** (`DN3A.5`). Aquele teste significava duas coisas incompatíveis — "é de nuvem, RAM é de graça" e "não consegui ler a atenção deste modelo local" —, e sob a segunda os dois `qwen3.5` **locais** recebiam o teto treinado de 262.144 numa máquina que aguenta uma fração. Hoje é `offeredCeiling(model, freeBytes, marginBytes)` e `costsLocalRam(model)`, os dois em `core/ai/memory.ts`. ⚠️ **Há DOIS chamadores, não um**: `ConversationView` e `useResolvedContextWindow` (o caminho do `ai:propose`) — consertar só o primeiro deixa a ambiguidade viva.

⚠️ **`qwen3.5:4b` publica um bloco `qwen35.vision.*` paralelo** — `vision.attention.head_count`, `vision.block_count`, `vision.embedding_length`. É exatamente o caso que faz `readInfo` casar **por caminho abaixo do prefixo de família**, descartando um segmento, em vez de por sufixo: um `.block_count` de visão coexiste com o real. A disciplina de D15.8 sendo exercitada de novo, por um modelo que ela não previa.
...
- **`OVERHEAD = 1.06`** — medido, não derivado: 38,0 KB por token contra os 36,0 que a fórmula prevê, no `qwen2.5-coder:3b` (D15.8). **`HYBRID_OVERHEAD = 1.2`** para a família híbrida, onde o resíduo medido é 1,13–1,18 e aponta para **subestimar** (`DN3A.8`).
...
- **`growingLayers`** — todas as camadas crescem com `num_ctx` **sem** sliding window ativa; **uma só** cresce com ela ativa (empírico, `gemma3:4b`, fator 1,07 medido); e num **híbrido**, `ceil(block_count / full_attention_interval)` — as demais camadas são recorrentes, de estado fixo (`DN3A.7`). A janela é comparada ao **teto do próprio modelo**, nunca ao `numCtx` candidato sendo calculado — senão a função seria recursiva sobre a própria saída (D15.8).
...
⚠️ **Capacidade vem de `/api/show`, nunca de `/api/tags`.** `/api/tags` omite `vision` por completo (`gemma3:4b` é `["completion"]` ali, `["completion","vision"]` em `/api/show`) enquanto relata `tools` nos dois — um gate construído sobre `tags` recusaria todo modelo com visão da máquina. É por isso que `ollamaModels` faz N+1 requisições (`/api/tags` uma vez, `/api/show` por modelo) em vez de confiar só no primeiro.
...
⚠️ **Lista fixa de modelos, aplicada ANTES do `/api/show`** (`OLLAMA_CLOUD_MODEL_NAMES`, `core/ai/models.ts`): `/api/tags` traz 20 modelos, 14 respondem **402**, e o catálogo autenticado é idêntico ao anônimo — nada na API separa os gratuitos. Filtrar antes paga o N+1 (1 + 2, não 1 + 20). A lista envelhece, e isso é aceito: um modelo que suma não aparece; um modelo novo e bom **não entra sozinho**. São **dois** desde o `N-3-D` — `gemma4:31b` (o padrão, único com visão) e `gpt-oss:120b`; o `gpt-oss:20b` fica fora por HTTP 400 em toda requisição com `tools` (`DNC-4`).
...
| Sinal de "pensar" | `think: thinkLevel ?? onThinking !== undefined` | `includeThoughts: true` dentro de `thinkingConfig` | `thinking: { type: 'enabled' \| 'disabled' }` |
...
⚠️ **Mas há uma família em que o booleano NÃO é o sinal, e mandar `false` nela é o caminho caro.** O `api/types.go` do Ollama declara `Think` como booleano **ou** nível, e a doc é textual: `gpt-oss` exige `low`/`medium`/`high`, e **não tem desligado** — `think: false` ali é ignorado, e o modelo pensa no nível **padrão**, 7× os tokens da mesma resposta. Por isso `requiredThinkLevel(name)` (`core/ai/models.ts`) devolve `'low'` para essa família **nos dois estados do interruptor** (`DN3D.3`), e o handler é quem preenche `ChatFn.thinkLevel` (`DN3D.1`). ⚠️ **O predicado é por NOME de família, e não há alternativa:** `capabilities` diz `thinking` tanto para quem aceita booleano quanto para quem exige nível (`DN3D.2`). Quem garante que o rastro não pedido não aparece continua sendo a guarda do `N-3-A`, não o pedido.

⚠️ **`exposesReasoning` não muda com isso** — `gpt-oss` streama `message.thinking` normalmente. O que muda é que "desligado" significa *não exibido*, nunca *não gerado*, e o app não promete o que o provedor não faz (`DNC-13`).

⚠️ **Aceitar um nível não é honrá-lo, e o teste óbvio responde sim para todos.** O daemon **valida** o `ThinkValue` — `think: "banana"` devolve 400 nomeando o enum —, então `200` prova só que a string é legal. Medido em 14/09/2026 com `seed` fixa: os dois `qwen3.5` devolvem saída **idêntica byte a byte** para `true`/`'low'`/`'medium'`/`'high'`. **Nenhum modelo da frota local honra nível**; escolher esforço é da trilha `N-2`, e o levantamento por provedor, o método de sonda e o desenho aprovado têm dono: [`reference/reasoning/`](../../../docs/reference/reasoning/README.md) § *Esforço de pensamento*.
...
⚠️ **`total_duration` é a única duração que a nuvem manda**, e o app a lê desde o `N-3-D` (`DNC-20`): as três nativas (`load_duration`, `prompt_eval_duration`, `eval_duration`) só existem no daemon local, então o painel Desempenho mostra `—` em `Entrada tok/s` e `Carga (pico)` para todo provedor de nuvem — ausência é a resposta verdadeira, nunca zero. O total é do **servidor**, então a diferença contra o relógio de parede do app é a rede (~1,1 s medido). ⚠️ A média dele cobre **só as linhas que o carregam**, então comparar colunas do painel só vale num balde cujas linhas todas o tenham — resposta anterior à migração `v5` fica `NULL` para sempre.

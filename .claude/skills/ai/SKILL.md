...
O que decide escolha, em uma linha cada: `gemma3:4b` é o **default**, com visão · `gemma3:1b` é o fallback de baixa RAM · `qwen2.5-coder:3b` é o candidato a default do NL→SQL (único que junta código e folga de RAM) · `qwen3:4b` tem `thinking`, com o maior teto treinado e o cache mais caro da frota · `qwen3.5:2b` tem `thinking` **e** visão, e é hoje o modelo mais usado na prática — o Observatório (painel Desempenho) mostra mediana de ~38 tok/s de saída contra ~5 tok/s dos modelos maiores da frota.

⚠️ **`gemma3:4b` deixou de ser o único com visão, e a documentação em `docs/reference/models/` ainda não foi remedida para refletir isso** — `qwen3.5:2b` também declara `vision`, achado ao vivo, não confirmado ainda contra `/api/show`. Reconfira `capabilities` do modelo antes de repetir "único com X" em qualquer lugar — a frota muda mais rápido do que os documentos que a descrevem. Ficha técnica completa de cada um (peso, teto, KV/token): [`docs/reference/models/`](../../../docs/reference/models/README.md) — esta skill decide qual usar, aquele diretório decide o custo de usá-lo.

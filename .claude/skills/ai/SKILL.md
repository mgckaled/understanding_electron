...
⚠️ **A dupla `qwen3.5` tem `attention.head_count_kv: null` — `readAttention()` (abaixo) não consegue custear o contexto de nenhum dos dois.** Mesmo defeito documentado para a arquitetura híbrida Mamba (`ARMADILHAS.md`), mas estes estão na frota e são os mais usados — o orçamento de RAM/contexto silenciosamente não faz conta nenhuma para eles. Não corrigido; é a frente 2 da **F-6**.

> 🔍 **E o `qwen3.5:4b` (sondado em 13/09/2026) transforma o que parecia caso isolado em propriedade da família.** `head_count = 16` e `head_count_kv = null` nos dois, na arquitetura `qwen35` — então a hipótese que a F-6 registra (usar `head_count` quando `head_count_kv` falta) passa a valer para **dois** modelos, não um, e a correção paga o dobro. ⚠️ **Não a codifique direto:** `head_count` só substitui `head_count_kv` se o modelo usar MHA completa, e isso continua por verificar.

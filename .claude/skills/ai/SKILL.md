...
O que decide escolha, em uma linha cada: `gemma3:4b` é o **default**, com visão · `gemma3:1b` é o fallback de baixa RAM · `qwen2.5-coder:3b` é o candidato a default do NL→SQL (único que junta código e folga de RAM) · `qwen3:4b` tem `thinking`, com o maior teto treinado e o cache mais caro da frota · `qwen3.5:2b` tem `completion`, `vision`, `tools` **e** `thinking` juntas — primeira vez na frota com as quatro —, e é o modelo mais usado na prática: mediana de ~38 tok/s de saída no painel Desempenho do Observatório, contra ~5 tok/s dos modelos maiores.

⚠️ **`gemma3:4b` não é mais o único com visão.** Reconfira `capabilities` (`/api/show`) antes de repetir "único com X" em qualquer lugar — a frota muda mais rápido do que os documentos que a descrevem, e `docs/reference/models/` ainda não tem o reinventário completo desde que `qwen3.5:2b` entrou. Ficha técnica completa de cada um (peso, teto, KV/token): [`docs/reference/models/`](../../../docs/reference/models/README.md) — esta skill decide qual usar, aquele diretório decide o custo de usá-lo.

⚠️ **`qwen3.5:2b` tem `attention.head_count_kv: null` — `readAttention()` (abaixo) não consegue custear o contexto deste modelo.** Mesmo defeito documentado para a arquitetura híbrida Mamba (`ARMADILHAS.md`), mas este modelo está na frota e é o mais usado — o orçamento de RAM/contexto silenciosamente não faz conta nenhuma para ele. Não corrigido.

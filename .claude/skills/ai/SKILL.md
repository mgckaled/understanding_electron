...
⚠️ **Esforço de pensamento existe em TRÊS dos quatro serviços, com conjuntos de valores diferentes** — medido em 14/09/2026, e é o que justifica o corte `N-3-E`: Ollama (`low`/`medium`/`high`/`max`, e a maioria dos modelos aceita booleano **ou** nível), Gemini (`thinking_level` com quatro valores, **variáveis por modelo** — o `minimal` que falha no `gemini-3.7-flash` existe nos Flash 3.x, então o `'low'` chumbado ali nunca foi testado contra os outros), GLM (**nenhum**: só `enabled`/`disabled`). Na frota **local**, só os dois `qwen3.5` têm `thinking`, e se aceitam nível **não está medido**.

...
⚠️ **`MarkdownMessage` não é "o corpo da mensagem do modelo" — é o dono do markdown no app, e todo texto de fora passa por ele.** Desde o 23-F ele também desenha a **descrição de um trecho do Context7** e o **texto de uma nota**, que não são mensagem nenhuma. O critério não é o tamanho nem o lugar: é a **procedência**. Texto que veio de fora da máquina chega com marcação (a API do Context7 escreve código inline em acento grave) e é entrada não confiável — o mesmo modelo de ameaça de D11.2, que este primitivo já responde por não ter `rehype-raw` e por passar todo link por `checkExternalUrl`.

**A tentação a recusar é um partidor de acento grave de oito linhas.** Ele resolve o que se vê e cria um segundo dono do assunto, que envelhece calado — é a regra de fonte única aplicada a markdown. Se o custo do `react-markdown` numa legenda de uma linha incomodar, a resposta é medir, não escrever um parser. ⚠️ O primitivo fixa `font-size: var(--font-size-reading)` na própria folha, então quem o usa como legenda **herda 18px** e pode ficar maior que o título que legenda; o conserto é subir o título, nunca descer o corpo por dentro do módulo.

⚠️ **Ele tem DUAS formas, e a segunda existe para texto de fora que vive dentro de um controle** (`inline`, 23-J). O título do trecho do Context7 mora no `<button>` do retrátil, com `truncate`: um `<p>` de bloco em 18px fixos ali quebra a linha. A forma inline devolve um `<span>` (`disallowedElements={['p']}` + `unwrapDisallowed`, a API documentada do `react-markdown`) e **não declara `font-size`** — herdar o tamanho de quem chama **é** o ponto, e é a única diferença que importa entre as duas. A tentação recusada continua sendo a mesma: um partidor de acento grave à parte criaria um segundo dono do markdown.

⚠️ **E a régua "se o custo incomodar, meça" foi cobrada logo na estreia.** Um título renderiza dentro de árvores que rerenderizam a cada tecla do composer, e o **parse** refeito a cada render pôs `contextBudget.test.tsx` em 21,2 s contra 18,7 s — dois testes cruzaram o teto de 5 s **sob a suíte inteira**, passando isolados. O conserto foi `memo` no primitivo (voltou a 19,2 s), não um parser à mão nem um timeout maior. **Ao dar ao primitivo um chamador que vive numa árvore quente, meça antes de supor que não custa** — [`ARMADILHAS.md`](../../../docs/ARMADILHAS.md).

...
⚠️ **E há um lugar onde ele hoje não cabe, com pendência aberta:** texto de fora que vive **dentro de um controle** — o título do trecho do Context7 mora no `<button>` do retrátil, com `truncate`. Um `<p>` de bloco em 18px fixos ali quebra o layout, e o título **é** markdown (a API escreve código inline em acento grave), então sai cru desde o 23-G. A resposta não é um partidor de acento grave à parte: é uma **variante inline do próprio primitivo** (`span` em vez de `p`, herdando o tamanho de quem chama), que preserva a fonte única e serve qualquer legenda futura. A implementar no 23-K — [`ROADMAP § 4`](../../../docs/ROADMAP.md).

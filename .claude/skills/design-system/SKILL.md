...
`Button`, `Field`, `Panel`, `Toolbar`, `Dialog`, `Popover`, `MarkdownMessage`, `Switch` e `Slider` em `src/renderer/src/shared/ui/<Nome>/`. Os dois últimos entraram no plano F-2 (ago/2026, `docs/plan/active/F-2-composer-modelo-sidebar.md`) — nenhum limite físico neles, então nenhum ganhou CSS Module.
